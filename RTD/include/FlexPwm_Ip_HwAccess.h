...
#define FLEXPWM_IP_HWACCESS_AR_RELEASE_MINOR_VERSION     9
#define FLEXPWM_IP_HWACCESS_AR_RELEASE_REVISION_VERSION  0
#define FLEXPWM_IP_HWACCESS_SW_MAJOR_VERSION             7
#define FLEXPWM_IP_HWACCESS_SW_MINOR_VERSION             0
#define FLEXPWM_IP_HWACCESS_SW_PATCH_VERSION             1
...
/*FUNCTION**********************************************************************
 *
 * Function Name : FlexPwm_Ip_ClearFltInterruptHw
 * Description   : Disable CPU interrupt requests generated by the FAULTx pins. (CTRL register)
 *
 * Implements    : FlexPwm_Ip_ClearFltInterruptHw
 *END**************************************************************************/
static inline void FlexPwm_Ip_ClearFltInterruptHw(FlexPwm_Ip_HwAddrType *const Base, uint8 Value)
{
    #ifdef FLEXPWM_AE_IP_HW
    Base->FCTRL &= ~FLEXPWM_FCTRL_FIE(Value);
    #else
    Base->FP[0].CTRL &= ~FLEXPWM_CTRL_FIE(Value);
    #endif
}
...
{
    #ifdef FLEXPWM_AE_IP_HW
    Base->FSTS = (Base->FSTS & ~((uint16)FLEXPWM_FSTS_FFLAG_MASK)) | FLEXPWM_FSTS_FFLAG(FltFlag);
    #else
    Base->FP[0].STS = (Base->FP[0].STS & ~((uint16)FLEXPWM_STS_FFLAG_MASK)) | FLEXPWM_STS_FFLAG(FltFlag);
    #endif
}
 ...
 *END**************************************************************************/
static inline void FlexPwm_Ip_SetResetSubHw(FlexPwm_Ip_HwAddrType *const Base, uint8 SubModule, boolean IsAutosarUsed)
{
    #ifdef FLEXPWM_AE_IP_HW
    if (FALSE == IsAutosarUsed)
    {
