...
#define DIO_AR_RELEASE_MINOR_VERSION_C       9
#define DIO_AR_RELEASE_REVISION_VERSION_C    0
#define DIO_SW_MAJOR_VERSION_C               7
                                                        ...
                                                        )
{
    ...
    {
        u32ChannelToPartitionMap = Dio_Config.pau32Dio_ChannelToPartitionMap[ChannelId];
#if (STD_ON == DIO_MULTIPARTITION_ENABLED)
                                                     ...
                                                     )
{
       ...
       )
    {
        ...
        {
#endif /* (STD_ON == DIO_MULTIPARTITION_ENABLED) */
            if ((uint32)1 == ((Dio_Config.pau32Dio_PortToPartitionMap[PortId] & ((uint32)1 << PartitionId)) >> PartitionId))
            {
                Valid = (Std_ReturnType)E_OK;
            }
            else
                                                             ...
                                                             )
{
    ...
    else
    {
        ...
        if ((((pointerSizeType)(pChannelGroup)) >= ((pointerSizeType)Dio_Config.pChannelGroupList))                                  &&  \
            (((pointerSizeType)(pChannelGroup)) <= (pointerSizeType)(&Dio_Config.pChannelGroupList[Dio_Config.u8NumChannelGroups - 1U])) &&  \
...
Dio_PortLevelType Dio_ReadChannelGroup(const Dio_ChannelGroupType * ChannelGroupIdPtr)
{
    ...
    else
    {
        ...
        {
#ifdef DIO_CHANNEL_GROUPS_AVAILABLE
            for (GroupIndex = 0; GroupIndex < Dio_Config.u8NumChannelGroups; GroupIndex++)
            {
