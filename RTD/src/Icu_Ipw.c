...
#define ICU_IPW_AR_RELEASE_MINOR_VERSION_C      9
#define ICU_IPW_AR_RELEASE_REVISION_VERSION_C   0
#define ICU_IPW_SW_MAJOR_VERSION_C              7
...
void Icu_Ipw_SetSleepMode(const Icu_Ipw_ChannelConfigType* ChannelConfig)
{
    ...
    switch(ipType)
    {
        case ICU_EMIOS_MODULE:
            {
#if (STD_ON == EMIOS_ICU_IP_USED)
                Emios_Icu_Ip_DisableInterrupt(ChannelConfig->instanceNo, (ChannelConfig->pEmiosHwChannelConfig)->hwChannel);
#endif  /* EMIOS_ICU_IP_USED */
                break;
            }
        case ICU_SIUL2_MODULE:
            {
#if (STD_ON == SIUL2_ICU_IP_USED)
                Siul2_Icu_Ip_DisableInterrupt(ChannelConfig->instanceNo, (ChannelConfig->pSiul2HwChannelConfig)->hwChannel);
#endif  /* SIUL2_ICU_IP_USED */
                break;
            }
        case ICU_WKPU_MODULE:
            {
#if (STD_ON == WKPU_IP_USED)
                Wkpu_Ip_DisableInterrupt(ChannelConfig->instanceNo, (ChannelConfig->pWkpuHwChannelConfig)->hwChannel);
#endif  /* WKPU_IP_USED */
                break;
            }
        case ICU_CMP_MODULE:
...
void Icu_Ipw_SetNormalMode (const Icu_Ipw_ChannelConfigType* ChannelConfig)
{
    ...
    switch(ipType)
    {
        case ICU_EMIOS_MODULE:
            {
#if (STD_ON == EMIOS_ICU_IP_USED)
                Emios_Icu_Ip_EnableInterrupt(ChannelConfig->instanceNo, (ChannelConfig->pEmiosHwChannelConfig)->hwChannel);
#endif  /* EMIOS_ICU_IP_USED */
                break;
            }
        case ICU_SIUL2_MODULE:
            {
#if (STD_ON == SIUL2_ICU_IP_USED)
                Siul2_Icu_Ip_EnableInterrupt(ChannelConfig->instanceNo, (ChannelConfig->pSiul2HwChannelConfig)->hwChannel);
#endif  /* SIUL2_ICU_IP_USED */
                break;
            }
        case ICU_WKPU_MODULE:
            {
#if (STD_ON == WKPU_IP_USED)
                Wkpu_Ip_EnableInterrupt(ChannelConfig->instanceNo, (ChannelConfig->pWkpuHwChannelConfig)->hwChannel);
#endif  /* WKPU_IP_USED */
                break;
            }
        case ICU_CMP_MODULE:
    ...
    for(index = 0; index < modulesNumber; index++)
    {
        ...
        switch(ipType)
        {
            case ICU_EMIOS_MODULE:
                {
#if (STD_ON == EMIOS_ICU_IP_USED)
                    Emios_Icu_Ip_SetClockMode((*moduleConfig)[index].instanceNo, \
                        (*moduleConfig)[index].pEmiosHwIpConfig, \
                        (selectPrescaler == ICU_ALTERNATE_CLOCK_MODE)?(eMios_Icu_Ip_ClockModeType)EMIOS_ICU_ALTERNATE_CLK:(eMios_Icu_Ip_ClockModeType)EMIOS_ICU_NORMAL_CLK);
#endif  /* EMIOS_ICU_IP_USED */
                    break;
                }
            case ICU_SIUL2_MODULE:
                {
#if (STD_ON == SIUL2_ICU_IP_USED)
                    Siul2_Icu_Ip_SetClockMode((*moduleConfig)[index].instanceNo, \
                        (selectPrescaler == ICU_ALTERNATE_CLOCK_MODE)?(Siul2_Icu_Ip_ClockModeType)SIUL2_ICU_ALTERNATE_CLK:(Siul2_Icu_Ip_ClockModeType)SIUL2_ICU_NORMAL_CLK);
#endif  /* SIUL2_ICU_IP_USED */
                    break;
                }
            default:
