...
#define SWT_IP_AR_RELEASE_MINOR_VERSION_C       9
#define SWT_IP_AR_RELEASE_REVISION_VERSION_C    0
#define SWT_IP_SW_MAJOR_VERSION_C               7
#define SWT_IP_SW_MINOR_VERSION_C               0
#define SWT_IP_SW_PATCH_VERSION_C               1
...
#error "Software Version Numbers of Swt_Ip.c and Swt_Ip_Irq.h are different"
    ...
    #error "AutoSar Version Numbers of Swt_Ip.c and Std_Types.h are different"
    #endif
...
static Swt_Ip_Type * const Swt_Ip_apxBase[SWT_INSTANCE_NUM] = SWT_IP_SWT_BASE_PTRS_USED;
...
static Swt_Ip_StatusType Swt_Ip_Unlock(Swt_Ip_Type * const Base)
{
    ...
    else
    {
        ...
        if ((Base->CR & SWT_CR_SLK_MASK) != 0U)
        {
            ...
            {
                /* Check if the unlock sequence was successful after timeout expired.
                 * If the unlock sequence was successful, enable soft lock again and return error status.
                 * This prevents the unlock sequence from being successful after the OSIF timeout expired. */
                if ((Base->CR & SWT_CR_SLK_MASK) == 0U)
                {
...
static inline uint16 Swt_Ip_ServiceKeyGen(const Swt_Ip_Type * const Base)
{
    ...
    return ((uint16)(((uint32)((((Base->SK & SWT_SK_SK_MASK) >> SWT_SK_SK_SHIFT) * 17U) + 3U)) & SWT_SK_SK_MASK));
}
...
Swt_Ip_StatusType Swt_Ip_Init(const uint32 Instance,
                              const Swt_Ip_ConfigType * const ConfigPtr)
{
    Swt_Ip_Type * const Base = Swt_Ip_apxBase[Instance];
    Swt_Ip_StatusType Ret = SWT_IP_STATUS_SUCCESS;
...
*/
    ...
    {
        Swt_Ip_Type * const Base = Swt_Ip_apxBase[Instance];
        const uint32 Swt_Ip_au32CrResetValue[SWT_INSTANCE_NUM] = SWT_IP_CR_RESET_VALUE_U32;
        const uint32 Swt_Ip_au32ToResetValue[SWT_INSTANCE_NUM] = SWT_IP_TO_RESET_VALUE_U32;
        ...
        if (SWT_IP_STATUS_SUCCESS == Ret)
        {
            ...
            Base->TO = Swt_Ip_au32ToResetValue[Instance];
            /* Resets window value */
            Base->WN = SWT_IP_WN_RESET_VALUE_U32;
            /* Resets service key value */
            Base->SK = SWT_IP_SK_RESET_VALUE_U16;
            /* Resets control register */
            Base->CR = Swt_Ip_au32CrResetValue[Instance] & (~SWT_CR_WEN_MASK); /* make sure the SWT is disabled after reset */
                                ...
                                const Swt_Ip_ConfigType * const ConfigPtr)
{
    ...
    if (SWT_IP_STATUS_SUCCESS == Ret)
    {
        /* Workaround for ERR052226*/
        #if (SWT_IP_SUPPORT_ERR052226 == STD_ON)
            /* Check if it is in window mode */
            if (((Base->CR & SWT_CR_WND_MASK) >> SWT_CR_WND_SHIFT) != 0U)
            {
                if (Swt_Ip_Unlock(Base) == SWT_IP_STATUS_SUCCESS)
                {
                    /* Disable window mode */
...
void Swt_Ip_IrqHandler(uint32 Instance)
{
    ...
    {
        /* CPR_RTD_00012: If the driver is not initialized, the ISR should only clear the interrupt status flag for interrupts that are enabled. */
        if (Swt_Ip_IsIntEnabled(Base))
        {
            Swt_Ip_ClearIntFlag(Base);
        }
    }
