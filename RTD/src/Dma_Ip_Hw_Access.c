...
#define DMA_IP_HW_ACCESS_AR_RELEASE_MINOR_VERSION_C        9
#define DMA_IP_HW_ACCESS_AR_RELEASE_REVISION_VERSION_C     0
#define DMA_IP_HW_ACCESS_SW_MAJOR_VERSION_C                7
#define DMA_IP_HW_ACCESS_SW_MINOR_VERSION_C                0
#define DMA_IP_HW_ACCESS_SW_PATCH_VERSION_C                1
...
==================================================================================================*/
#if (STD_ON == DMA_IP_HWV3_IS_AVAILABLE)
    #if (STD_ON == DMA_IP_TCD_HWV3_NOT_ALIGNED)
        Dma_Ip_Hwv3ChTcdRegType      * const g_ptHwChTcdPtrArray[DMA_IP_TCD_NOF_HWV3_CH_INST][DMA_IP_TCD_NOF_HWV3_CH] = DMA_IP_TCD_PTR_ARRAY;
    #else
        Dma_Ip_Hwv3TcdArrayType      * const g_ptHwv3BaseTcdPtrArray[DMA_IP_TCD_NOF_HWV3_CH_INST] = DMA_IP_TCD_BASE_PTRS;
    #endif
...
void HwAccDmaInst_GetHRSStatus(const uint32 LocHwVers, const uint32 LocHwInst, const uint32 LocHwCh, boolean * const Status)
{
    uint32 LocRetVal;

    /* Check that shift operation's operand respects 32 bit boundary. */
    DMA_IP_DEV_ASSERT(LocHwCh <= 32U);

    switch (LocHwVers)
    {
#if (STD_ON == DMA_IP_HWV3_IS_AVAILABLE)
        case DMA_IP_HARDWARE_VERSION_3:
            hwv3AccInlineDmaInst_GetHRSActiveStatus(Dma_Ip_paxHwv3BaseInst[LocHwInst], &LocRetVal);
        break;
#endif /* #if (STD_ON == DMA_IP_HWV3_IS_AVAILABLE) */
    ...
    /* Isolate the channel specific HRS bit and get it's value. */
    *Status = ((LocRetVal & (1UL << LocHwCh)) != 0UL) ? TRUE : FALSE;
}
...
#if (STD_ON == DMA_IP_HWV3_IS_AVAILABLE)
        case DMA_IP_HARDWARE_VERSION_3:
            hwv3AccInlineDmaCh_CmdClearInterruptStatus(&DMA_IP_HWV3_TCD_CH_VALUE(LocHwInst, LocHwCh));
        break;
#endif /* #if (STD_ON == DMA_IP_HWV3_IS_AVAILABLE) */
...
static void HwAccDmaCh_GetStoreDstAddress(const uint32 LocHwVers, const uint32 LocHwInst, const uint32 LocHwCh, uint32 * const retValue)
{
    ...
    switch (LocHwVers)
    {
        ...
        #if (STD_ON == DMA_IP_VIRTUAL_ADDRESS_MAPPING_IS_AVAILABLE)
            *retValue = *((Dma_Ip_uintPtrType*)((Dma_Ip_uintPtrType)HwAccDma_SetPhysicalToVirtualAddress(LocPhysicalAddress)));
        #else
...
static uint32 HwAccDma_SetVirtualToPhysicalAddress(const uint32 VirtualAddr)
{
    ...
    #ifdef DMA_IP_RAM_END_R_CORE
    DMA_IP_DEV_ASSERT(PhysicAddr >= DMA_IP_RAM_START_R_CORE);
    DMA_IP_DEV_ASSERT(PhysicAddr <= DMA_IP_RAM_END_R_CORE);
    #endif /* #ifdef DMA_IP_RAM_END_R_CORE */
#elif (CPU_TYPE == CPU_TYPE_32)
    #ifdef DMA_IP_RAM_END_M_CORE
    DMA_IP_DEV_ASSERT(PhysicAddr <= DMA_IP_RAM_END_M_CORE);
    #endif /* #ifdef DMA_IP_RAM_END_M_CORE */
    #ifdef DMA_IP_RAM_START_M_CORE
    DMA_IP_DEV_ASSERT(DMA_IP_RAM_START_M_CORE < PhysicAddr);
    #endif /* #ifdef DMA_IP_RAM_START_M_CORE */
#else
...
#if (CPU_TYPE == CPU_TYPE_64)
    #ifdef DMA_IP_RAM_END_R_CORE
    DMA_IP_DEV_ASSERT(PhysicalAddr >= DMA_IP_RAM_START_R_CORE);
    DMA_IP_DEV_ASSERT(PhysicalAddr <= DMA_IP_RAM_END_R_CORE);
    #endif /* #ifdef DMA_IP_RAM_END_R_CORE */
#elif (CPU_TYPE == CPU_TYPE_32)
    #ifdef DMA_IP_RAM_END_M_CORE
    DMA_IP_DEV_ASSERT(PhysicalAddr <= DMA_IP_RAM_END_M_CORE);
    #endif /* #ifdef DMA_IP_RAM_END_M_CORE */
    #ifdef DMA_IP_RAM_START_M_CORE
    DMA_IP_DEV_ASSERT(DMA_IP_RAM_START_M_CORE < PhysicalAddr);
    #endif /* #ifdef DMA_IP_RAM_START_M_CORE */
#else
    #error "Unsupported CPU_TYPE"
#endif
