...
#define EMIOS_PWM_IP_AR_RELEASE_MINOR_VERSION_C       9
#define EMIOS_PWM_IP_AR_RELEASE_REVISION_VERSION_C    0
#define EMIOS_PWM_IP_SW_MAJOR_VERSION_C               7
...
static Emios_Pwm_Ip_PwmModeType Emios_Pwm_Ip_aInitialModes[EMIOS_PWM_IP_NUM_OF_CHANNELS_USED_U8] = EMIOS_PWM_IP_INITIAL_MODES;

/* Array with current pwm modes for each Emios Channel */
Emios_Pwm_Ip_PwmModeType Emios_Pwm_Ip_aCurrentModes[EMIOS_PWM_IP_NUM_OF_CHANNELS_USED_U8] = EMIOS_PWM_IP_INITIAL_MODES;
...
Emios_Pwm_Ip_NotificationType const *Emios_Pwm_Ip_aNotificationPtr[EMIOS_PWM_IP_NUM_OF_CHANNELS_USED_U8];
...
Emios_Pwm_Ip_PeriodType Emios_Pwm_Ip_aPeriod[EMIOS_PWM_IP_NUM_OF_CHANNELS_USED_U8];

#ifdef EMIOS_PWM_IP_MODE_OPWMCB_USED
/** @brief Array with deadtime for each channels */
static Emios_Pwm_Ip_PeriodType Emios_Pwm_Ip_aDeadTime[EMIOS_PWM_IP_NUM_OF_CHANNELS_USED_U8];
#endif
...
static Emios_Pwm_Ip_PeriodType Emios_Pwm_Ip_aRegA[EMIOS_PWM_IP_NUM_OF_CHANNELS_USED_U8];
#endif

/** @brief Array with duty cycle for all channels */
Emios_Pwm_Ip_DutyType Emios_Pwm_Ip_aDutyCycle[EMIOS_PWM_IP_NUM_OF_CHANNELS_USED_U8];
#ifdef EMIOS_PWM_IP_TIMER_WIDTH_24BITS
#if (EMIOS_PWM_IP_NO_CACHE_NEEDED == STD_ON)
...
static uint8 Emios_Pwm_Ip_aNotif[EMIOS_PWM_IP_NUM_OF_CHANNELS_USED_U8];

/** @brief Arrays to check the state of Channel in the notification or not */
uint8 Emios_Pwm_Ip_aCheckEnableNotif[EMIOS_PWM_IP_NUM_OF_CHANNELS_USED_U8];

#if (defined(EMIOS_PWM_IP_MODE_OPWFM_USED) || defined(EMIOS_PWM_IP_MODE_DAOC_USED))
/** @brief Arrays with the polarity of channels */
static uint8 Emios_Pwm_Ip_aPolarity[EMIOS_PWM_IP_NUM_OF_CHANNELS_USED_U8];
#endif

/** @brief Arrays to check the state of Channel is initial state or uninitialized state or idle state */
uint8 Emios_Pwm_Ip_aCheckState[EMIOS_PWM_IP_NUM_OF_CHANNELS_USED_U8];

#if (defined(EMIOS_PWM_IP_AUTOSAR_MODE_IS_USED) && (STD_ON == EMIOS_PWM_IP_AUTOSAR_MODE_IS_USED))
#if (defined(EMIOS_PWM_IP_MODE_OPWMCB_USED) || defined(EMIOS_PWM_IP_MODE_DAOC_USED) || defined(EMIOS_PWM_IP_MODE_OPWFM_USED) || defined(EMIOS_PWM_IP_MODE_OPWM_USED))
/* Arrays to used to monitor the usage of gpio output mode at init time */
static uint8 Emios_Pwm_Ip_aCheckGpioOutputModeUsedAtInit[EMIOS_PWM_IP_NUM_OF_CHANNELS_USED_U8];
#endif
#endif
...
#if (EMIOS_PWM_IP_NO_CACHE_NEEDED == STD_ON)
    #define PWM_START_SEC_VAR_INIT_8_NO_CACHEABLE
#else
...
#if (EMIOS_PWM_IP_NO_CACHE_NEEDED == STD_ON)
    #define PWM_STOP_SEC_VAR_INIT_8_NO_CACHEABLE
#else
    #define PWM_STOP_SEC_VAR_INIT_8
#endif
#include "Pwm_MemMap.h"

#ifdef EMIOS_PWM_IP_ASR_IPL_MIXED_CONTEXT
#if (EMIOS_PWM_IP_NO_CACHE_NEEDED == STD_ON)
    #define PWM_START_SEC_VAR_INIT_BOOLEAN_NO_CACHEABLE
#else
    #define PWM_START_SEC_VAR_INIT_BOOLEAN
#endif
#include "Pwm_MemMap.h"

/* Arrays to used to check which layer Emios_Pwm_Ip_DeInitChannel called in */
boolean eMios_Pwm_Ip_aDeInitChannelCalledByASR[EMIOS_PWM_IP_INSTANCE_COUNT]  = EMIOS_PWM_IP_INITIAL_ARRAY;

#if (EMIOS_PWM_IP_NO_CACHE_NEEDED == STD_ON)
    #define PWM_STOP_SEC_VAR_INIT_BOOLEAN_NO_CACHEABLE
#else
...
#if ((defined EMIOS_PWM_IP_MODE_OPWFM_USED) || (defined EMIOS_PWM_IP_MODE_OPWFMB_USED) || (defined EMIOS_PWM_IP_MODE_OPWMCB_USED) \
  || (defined EMIOS_PWM_IP_MODE_OPWMB_USED) || (defined EMIOS_PWM_IP_MODE_OPWM_USED)  || (defined EMIOS_PWM_IP_MODE_OPWMT_USED))
/*FUNCTION**********************************************************************
 *
 * Function Name : Emios_Pwm_Ip_CheckEnableNotifAndClearFlag
 * Description   : Clear the flag event if the flag enable of channel is set previously.
 *
 *END**************************************************************************/
static inline void Emios_Pwm_Ip_CheckEnableNotifAndClearFlag(uint8 Instance,
                                                             uint8 Channel
                                                            )
{
    Emios_Pwm_Ip_HwAddrType *const Base = Emios_Pwm_Ip_aBasePtr[Instance];

    if (1U == Emios_Pwm_Ip_aCheckEnableNotif[eMios_Pwm_Ip_IndexInChState[Instance][Channel]])
    {
                                               ...
                                               )
{
    ...
    return Ret;
     ...
     (defined EMIOS_PWM_IP_MODE_OPWMC_USED)  || \
     (defined EMIOS_PWM_IP_MODE_OPWMB_USED)  || \
     (defined EMIOS_PWM_IP_MODE_OPWM_USED)   || \
     (defined EMIOS_PWM_IP_MODE_OPWMT_USED)  || \
     (EMIOS_PWM_IP_DEV_ERROR_DETECT == STD_ON))
 ...
 * Only used for channels in OPWM/OPWMB/OPWMCB/OPWMT modes
 *
 *END**************************************************************************/
                                                                           ...
                                                                           )
{
    ...
    switch (CounterBus)
    {
        case EMIOS_PWM_IP_BUS_A:
            ...
            break;
        case EMIOS_PWM_IP_BUS_BCD:
            CounterBusMode = (Emios_Pwm_Ip_MasterBusModeType)Emios_Pwm_Ip_GetChannelPwmMode(Base, (Channel & (uint8)EMIOS_PWM_IP_COUNTER_BUS_BCD));
            break;
...
{
#if (EMIOS_PWM_IP_DEV_ERROR_DETECT == STD_ON)
    boolean ValidMode = Emios_Pwm_Ip_ValidateMode(Instance, UserChCfg->ChannelId, EMIOS_PWM_IP_HW_MODE_OPWFMB);
    ...
    else if(0U == UserChCfg->DutyCycle)
    {
        ...
        Emios_Pwm_Ip_aNotif[eMios_Pwm_Ip_IndexInChState[Instance][UserChCfg->ChannelId]] = (uint8)1U;
        /* Configure output pin polarity */
        Emios_Pwm_Ip_SetEdgePolarity(Base,
                                     UserChCfg->ChannelId,
                                     (UserChCfg->OutputPolarity == EMIOS_PWM_IP_ACTIVE_HIGH) ? (Emios_Pwm_Ip_PolarityType)EMIOS_PWM_IP_ACTIVE_LOW : (Emios_Pwm_Ip_PolarityType)EMIOS_PWM_IP_ACTIVE_HIGH );
    }
    else
    {
...
/*FUNCTION**********************************************************************
 *
 * Function Name : Emios_Pwm_Ip_InitModeWithDutyUseGpioOutputMode
 * Description   : Force the output of channel to the desired state based on the duty cycle and period requested
 * This function will configure the polarity of channel to force the output of channel to the desired state by using the gpio output mode.
 *
 *END**************************************************************************/
static void Emios_Pwm_Ip_InitModeWithDutyUseGpioOutputMode(uint8 Instance,
                                                           Emios_Pwm_Ip_ChannelConfigType const *UserChCfg
                                                          )
{
    Emios_Pwm_Ip_HwAddrType *const Base = Emios_Pwm_Ip_aBasePtr[Instance];
    ...
    if (
        ((UserChCfg->PeriodCount == UserChCfg->DutyCycle) && (UserChCfg->OutputPolarity  == EMIOS_PWM_IP_ACTIVE_HIGH)) ||
        (((0U == UserChCfg->DutyCycle) || ((1U == UserChCfg->DutyCycle))) && (UserChCfg->OutputPolarity  == EMIOS_PWM_IP_ACTIVE_LOW))
       )
    {   /* Logic 1 is desired. */
        Emios_Pwm_Ip_SetEdgePolarity(Base, UserChCfg->ChannelId, EMIOS_PWM_IP_ACTIVE_HIGH);
        Emios_Pwm_Ip_aCheckGpioOutputModeUsedAtInit[eMios_Pwm_Ip_IndexInChState[Instance][UserChCfg->ChannelId]] = 1U;
        Emios_Pwm_Ip_aNotif[eMios_Pwm_Ip_IndexInChState[Instance][UserChCfg->ChannelId]] = (uint8)1U;
    }
    else if (
             ((UserChCfg->PeriodCount == UserChCfg->DutyCycle) && (UserChCfg->OutputPolarity  == EMIOS_PWM_IP_ACTIVE_LOW)) ||
             (((0U == UserChCfg->DutyCycle) || ((1U == UserChCfg->DutyCycle))) && (UserChCfg->OutputPolarity  == EMIOS_PWM_IP_ACTIVE_HIGH))
            )
    {
        /* Logic 0 is desired. */
        Emios_Pwm_Ip_SetEdgePolarity(Base, UserChCfg->ChannelId, EMIOS_PWM_IP_ACTIVE_LOW);
        Emios_Pwm_Ip_aCheckGpioOutputModeUsedAtInit[eMios_Pwm_Ip_IndexInChState[Instance][UserChCfg->ChannelId]] = 1U;
        Emios_Pwm_Ip_aNotif[eMios_Pwm_Ip_IndexInChState[Instance][UserChCfg->ChannelId]] = (uint8)1U;
    }
    else
    ...
    #endif /* defined(EMIOS_PWM_IP_MODE_OPWMCB_USED) || defined(EMIOS_PWM_IP_MODE_DAOC_USED) || defined(EMIOS_PWM_IP_MODE_OPWFM_USED) || defined(EMIOS_PWM_IP_MODE_OPWM_USED) */
#endif /* (defined(EMIOS_PWM_IP_AUTOSAR_MODE_IS_USED) && (STD_ON == EMIOS_PWM_IP_AUTOSAR_MODE_IS_USED) */
...
{
#if (EMIOS_PWM_IP_DEV_ERROR_DETECT == STD_ON)
    boolean ValidMode = Emios_Pwm_Ip_ValidateMode(Instance, UserChCfg->ChannelId, EMIOS_PWM_IP_HW_MODE_OPWFM);
...
#if (defined(EMIOS_PWM_IP_AUTOSAR_MODE_IS_USED) && (STD_ON == EMIOS_PWM_IP_AUTOSAR_MODE_IS_USED))
    if ((EMIOS_PWM_IP_MODE_GPO == Emios_Pwm_Ip_aCurrentModes[eMios_Pwm_Ip_IndexInChState[Instance][UserChCfg->ChannelId]]) &&
        ((UserChCfg->PeriodCount == UserChCfg->DutyCycle) || (0U == UserChCfg->DutyCycle)))
    {
        Emios_Pwm_Ip_InitModeWithDutyUseGpioOutputMode (Instance, UserChCfg);
    }
    else
    {
#endif
        ...
        else if(0U == UserChCfg->DutyCycle)
        {
            ...
            Emios_Pwm_Ip_SetEdgePolarity(Base,
                                        UserChCfg->ChannelId,
                                        (UserChCfg->OutputPolarity == EMIOS_PWM_IP_ACTIVE_HIGH) ? (Emios_Pwm_Ip_PolarityType)EMIOS_PWM_IP_ACTIVE_HIGH : (Emios_Pwm_Ip_PolarityType)EMIOS_PWM_IP_ACTIVE_LOW );
            Emios_Pwm_Ip_aNotif[eMios_Pwm_Ip_IndexInChState[Instance][UserChCfg->ChannelId]] = (uint8)1U;
        }
        else
        {
        ...
        if (UserChCfg->PeriodCount == UserChCfg->DutyCycle)
        {
            ...
            Emios_Pwm_Ip_SetEdgePolarity(Base,
                                        UserChCfg->ChannelId,
                                        (UserChCfg->OutputPolarity == (Emios_Pwm_Ip_PolarityType)EMIOS_PWM_IP_ACTIVE_HIGH) ? (Emios_Pwm_Ip_PolarityType)EMIOS_PWM_IP_ACTIVE_HIGH : EMIOS_PWM_IP_ACTIVE_LOW );
        }
#if (defined(EMIOS_PWM_IP_AUTOSAR_MODE_IS_USED) && (STD_ON == EMIOS_PWM_IP_AUTOSAR_MODE_IS_USED))
    }
#endif
                                                                     ...
                                                                     )
{
    ...
    else
    {
        ...
        else
        {
            ...
            /* Clear the flag event if the flag enable of channel is set previously */
            Emios_Pwm_Ip_CheckEnableNotifAndClearFlag (Instance, Channel);
                                                       ...
                                                       )
{
    if ((Emios_Pwm_Ip_PeriodType)0U == Emios_Pwm_Ip_aPeriod[eMios_Pwm_Ip_IndexInChState[Instance][Channel]])
    {
        if (EMIOS_PWM_IP_MODE_GPO != Emios_Pwm_Ip_aCurrentModes[eMios_Pwm_Ip_IndexInChState[Instance][Channel]])
        {
            ...
            Emios_Pwm_Ip_SetEdgePolarity(Base,
                                         Channel,
                                         (Emios_Pwm_Ip_aPolarity[eMios_Pwm_Ip_IndexInChState[Instance][Channel]] == (uint16)1U) ? (Emios_Pwm_Ip_PolarityType)EMIOS_PWM_IP_ACTIVE_HIGH : (Emios_Pwm_Ip_PolarityType)EMIOS_PWM_IP_ACTIVE_LOW );
        }
    }
    else
    {
        ...
        else
        {
            if (EMIOS_PWM_IP_MODE_GPO != Emios_Pwm_Ip_aCurrentModes[eMios_Pwm_Ip_IndexInChState[Instance][Channel]])
            {
                ...
                Emios_Pwm_Ip_SetEdgePolarity(Base,
                                             Channel,
                                             (Emios_Pwm_Ip_aPolarity[eMios_Pwm_Ip_IndexInChState[Instance][Channel]] == (uint16)1U) ? (Emios_Pwm_Ip_PolarityType)EMIOS_PWM_IP_ACTIVE_LOW : (Emios_Pwm_Ip_PolarityType)EMIOS_PWM_IP_ACTIVE_HIGH );
            }
        }
    }
}
...
/*FUNCTION**********************************************************************
 *
 * Function Name : Emios_Pwm_Ip_InitEdgePolarityChannelDeadTimeMode
 * Description   : Set polarity edge for the channel in OPWMCB Mode
 * This function will set polarity edge for the channel in OPWMCB Mode based on the duty cycle requested
 *
 *END**************************************************************************/
static void Emios_Pwm_Ip_InitEdgePolarityChannelDeadTimeMode(uint8 Instance,
                                                             Emios_Pwm_Ip_ChannelConfigType const *UserChCfg
                                                            )
{
    Emios_Pwm_Ip_HwAddrType *const Base = Emios_Pwm_Ip_aBasePtr[Instance];
...
#if (defined(EMIOS_PWM_IP_AUTOSAR_MODE_IS_USED) && (STD_ON == EMIOS_PWM_IP_AUTOSAR_MODE_IS_USED))
    if ((EMIOS_PWM_IP_MODE_GPO == Emios_Pwm_Ip_aCurrentModes[eMios_Pwm_Ip_IndexInChState[Instance][UserChCfg->ChannelId]]) &&
        ((UserChCfg->PeriodCount == UserChCfg->DutyCycle) || (0U == UserChCfg->DutyCycle) || (1U == UserChCfg->DutyCycle)))
    {
        Emios_Pwm_Ip_InitModeWithDutyUseGpioOutputMode (Instance, UserChCfg);
    }
    else
...
{
#if (EMIOS_PWM_IP_DEV_ERROR_DETECT == STD_ON)
    boolean ValidModeMCB = Emios_Pwm_Ip_ValidateMode(Instance, UserChCfg->ChannelId, EMIOS_PWM_IP_HW_MODE_OPWMCB);
    boolean ValidModeMC = Emios_Pwm_Ip_ValidateMode(Instance, UserChCfg->ChannelId, EMIOS_PWM_IP_HW_MODE_OPWMC);
    ...
    DevAssert((TRUE == ValidModeMCB) || (TRUE == ValidModeMC));
    ...
    Emios_Pwm_Ip_InitEdgePolarityChannelDeadTimeMode (Instance, UserChCfg);
    ...
    /* Stores the inital deadtime in ticks */
    Emios_Pwm_Ip_aDeadTime[eMios_Pwm_Ip_IndexInChState[Instance][UserChCfg->ChannelId]] = UserChCfg->DeadTime;

#if (defined(EMIOS_PWM_IP_AUTOSAR_MODE_IS_USED) && (STD_ON == EMIOS_PWM_IP_AUTOSAR_MODE_IS_USED))
    if ((EMIOS_PWM_IP_MODE_GPO != Emios_Pwm_Ip_aCurrentModes[eMios_Pwm_Ip_IndexInChState[Instance][UserChCfg->ChannelId]]) ||
        ((EMIOS_PWM_IP_MODE_GPO == Emios_Pwm_Ip_aCurrentModes[eMios_Pwm_Ip_IndexInChState[Instance][UserChCfg->ChannelId]]) &&
         (UserChCfg->PeriodCount != UserChCfg->DutyCycle) && (0U != UserChCfg->DutyCycle) && (1U != UserChCfg->DutyCycle)))
    {
#endif
...
#ifdef EMIOS_PWM_IP_MODE_OPWMCB_USED
static inline void Emios_Pwm_Ip_DutyCycleOpwmcbLimitCheck(uint8 Instance,
                                                          uint8 Channel,
                                                          boolean IsFullDutyCycleChecked
                                                         )
{
    Emios_Pwm_Ip_HwAddrType *const Base = Emios_Pwm_Ip_aBasePtr[Instance];
    ...
    /* This statement is required to avoid limitation of 100% duty cycle (if call 0% to 100%) */
    if (TRUE == IsFullDutyCycleChecked)
    {
        if (
            (1U == Emios_Pwm_Ip_GetUCRegA(Base, Channel)) ||
            ((ChPeriod + 1U) == Emios_Pwm_Ip_GetUCRegA(Base, Channel))
           )
        {
                                                                     ...
                                                                     )
{
    ...
    if (ChannelIdx < EMIOS_PWM_IP_NUM_OF_CHANNELS_USED_U8)
    {
        ...
        else if(0U == NewDutyCycle)
        {
            ...
            /* Set the value of deadtime stored in B register to zero to avoid a limiation of 0% duty cycle */
            ...
            Emios_Pwm_Ip_DutyCycleOpwmcbLimitCheck (Instance, Channel, FALSE);
        }
        else if(NewDutyCycle == ((ChPeriod * 2U) - 2U))
        {
            ...
            Emios_Pwm_Ip_DutyCycleOpwmcbLimitCheck (Instance, Channel, TRUE);
        }
        else
        {
            /* Revert the value of deadtime stored in B register if reset previously due to requesting a 0%/100% duty cycle */
            Emios_Pwm_Ip_SetUCRegB(Base, Channel, Emios_Pwm_Ip_aDeadTime[ChannelIdx]);
...
{
#if (EMIOS_PWM_IP_DEV_ERROR_DETECT == STD_ON)
    boolean ValidMode = Emios_Pwm_Ip_ValidateMode(Instance, UserChCfg->ChannelId, EMIOS_PWM_IP_HW_MODE_OPWM);
...
{
#if (EMIOS_PWM_IP_DEV_ERROR_DETECT == STD_ON)
    boolean ValidMode = Emios_Pwm_Ip_ValidateMode(Instance, UserChCfg->ChannelId, EMIOS_PWM_IP_HW_MODE_OPWMB);
...
{
#if (EMIOS_PWM_IP_DEV_ERROR_DETECT == STD_ON)
    boolean ValidMode = Emios_Pwm_Ip_ValidateMode(Instance, UserChCfg->ChannelId, EMIOS_PWM_IP_HW_MODE_OPWMT);
...
{
#if (EMIOS_PWM_IP_DEV_ERROR_DETECT == STD_ON)
    DevAssert(EMIOS_PWM_IP_NUM_OF_CHANNELS_USED_U8 > eMios_Pwm_Ip_IndexInChState[Instance][UserChCfg->ChannelId]);
#endif
    Emios_Pwm_Ip_HwAddrType *const Base = Emios_Pwm_Ip_aBasePtr[Instance];
    ...
    if (ChannelIndex < EMIOS_PWM_IP_NUM_OF_CHANNELS_USED_U8)
    {
        if(0U == UserChCfg->DutyCycle)
        {
            ...
            if (EMIOS_PWM_IP_MODE_GPO != Emios_Pwm_Ip_aCurrentModes[ChannelIndex])
            {
                Emios_Pwm_Ip_SetEdgePolarity(Base, UserChCfg->ChannelId, UserChCfg->OutputPolarity);
            }
        }
...
{
#if (EMIOS_PWM_IP_DEV_ERROR_DETECT == STD_ON)
    boolean ValidMode = Emios_Pwm_Ip_ValidateMode(Instance, UserChCfg->ChannelId, EMIOS_PWM_IP_HW_MODE_DAOC);
    ...
    /* Clear the flag event */
    Emios_Pwm_Ip_ClearFlagEvent(Base, UserChCfg->ChannelId);
    /* FLAG event enabled */
                                                      ...
                                                      )
{
    if (((Emios_Pwm_Ip_PeriodType)0U == Emios_Pwm_Ip_aPeriod[ChannelIdx]) || ((Emios_Pwm_Ip_DutyType)0U == NewDutyCycle))
    {
        if (EMIOS_PWM_IP_MODE_GPO != Emios_Pwm_Ip_aCurrentModes[ChannelIdx])
        {
            Emios_Pwm_Ip_SetEdgePolarity(Base,
                                         Channel,
                                         (Emios_Pwm_Ip_aPolarity[ChannelIdx] == 1U) ? (Emios_Pwm_Ip_PolarityType)EMIOS_PWM_IP_ACTIVE_HIGH : (Emios_Pwm_Ip_PolarityType)EMIOS_PWM_IP_ACTIVE_LOW);
        }
    }
    else if(NewDutyCycle == Emios_Pwm_Ip_aPeriod[ChannelIdx])
    {
        if (EMIOS_PWM_IP_MODE_GPO != Emios_Pwm_Ip_aCurrentModes[ChannelIdx])
        {
            Emios_Pwm_Ip_SetEdgePolarity(Base,
                                         Channel,
                                         (Emios_Pwm_Ip_aPolarity[ChannelIdx] == 1U) ? (Emios_Pwm_Ip_PolarityType)EMIOS_PWM_IP_ACTIVE_LOW : (Emios_Pwm_Ip_PolarityType)EMIOS_PWM_IP_ACTIVE_HIGH);
        }
    }
    else
    {
                             ...
                             )
{
    ...
    /* Get the value of channel used Index */
    uint8 ChannelUsedIdx = Emios_Pwm_Ip_GetChannelIndex(Instance, ChannelIndex);
    Emios_Pwm_Ip_HwAddrType *const Base = Emios_Pwm_Ip_aBasePtr[Instance];
    /* Get timbase channel */
...
#if (defined(EMIOS_PWM_IP_AUTOSAR_MODE_IS_USED) && (STD_ON == EMIOS_PWM_IP_AUTOSAR_MODE_IS_USED))
    if (EMIOS_PWM_IP_MODE_GPO == Emios_Pwm_Ip_aCurrentModes[ChannelUsedIdx])
    {
        /* Do not touch Polarity and Mode bits in control register to prevent spike */
    ...
    Emios_Pwm_Ip_aInitialModes[ChannelUsedIdx] = UserChCfg->Mode;
    /* Stores the current modes of Channel. */
    Emios_Pwm_Ip_aCurrentModes[ChannelUsedIdx] = UserChCfg->Mode;
    /* Confirm the Channel is active*/
    Emios_Pwm_Ip_aCheckState[ChannelUsedIdx] = (uint8)1U;
                               ...
                               )
{
    ...
    DevAssert(EMIOS_PWM_IP_INSTANCE_COUNT > Instance);
    DevAssert(EMIOS_PWM_IP_CHANNEL_COUNT > Channel);
    DevAssert(EMIOS_PWM_IP_NUM_OF_CHANNELS_USED_U8 > eMios_Pwm_Ip_IndexInChState[Instance][Channel]);
#endif
    Emios_Pwm_Ip_HwAddrType *const Base = Emios_Pwm_Ip_aBasePtr[Instance];
...
#ifdef EMIOS_PWM_IP_ASR_IPL_MIXED_CONTEXT
    if (TRUE == eMios_Pwm_Ip_aDeInitChannelCalledByASR[Instance])
    {
                                                 ...
                                                 )
{
    ...
    Emios_Pwm_Ip_PwmModeType ChMode;
    /* Channels shall be reactivated from idle state. */
    ChMode = ((uint8)2U == Emios_Pwm_Ip_aCheckState[eMios_Pwm_Ip_IndexInChState[Instance][Channel]]) ? Emios_Pwm_Ip_aInitialModes[eMios_Pwm_Ip_IndexInChState[Instance][Channel]] : \
                                                                                                       Emios_Pwm_Ip_aCurrentModes[eMios_Pwm_Ip_IndexInChState[Instance][Channel]];
                                                  ...
                                                  )
{
    ...
    Emios_Pwm_Ip_HwAddrType *const Base = Emios_Pwm_Ip_aBasePtr[Instance];
    Emios_Pwm_Ip_CounterBusSourceType CounterBus = Emios_Pwm_Ip_GetCounterBus(Base, Channel);
    Emios_Pwm_Ip_PeriodType ChPeriod;
    Emios_Pwm_Ip_DutyType DutyCycle;
#endif
...
#if (defined EMIOS_PWM_IP_MODE_OPWMCB_USED)
    /* Store the new dead time */
    Emios_Pwm_Ip_aDeadTime[eMios_Pwm_Ip_IndexInChState[Instance][Channel]] = NewDeadTime;
#endif
}
...
*
* @implements Emios_Pwm_Ip_SetOutputState_Activity
**/
...
*
* @implements Emios_Pwm_Ip_SetOutputToNormal_Activity
**/
...
#if (defined(EMIOS_PWM_IP_AUTOSAR_MODE_IS_USED) && (STD_ON == EMIOS_PWM_IP_AUTOSAR_MODE_IS_USED))
#if (defined(EMIOS_PWM_IP_MODE_OPWMCB_USED) || defined(EMIOS_PWM_IP_MODE_DAOC_USED) || defined(EMIOS_PWM_IP_MODE_OPWFM_USED) || defined(EMIOS_PWM_IP_MODE_OPWM_USED))
     || (1U == Emios_Pwm_Ip_aCheckGpioOutputModeUsedAtInit[eMios_Pwm_Ip_IndexInChState[Instance][Channel]])
#endif
#endif
       )
    {
     ...
     (defined EMIOS_PWM_IP_MODE_DAOC_USED))
        switch (Mode)
        {
#ifdef EMIOS_PWM_IP_MODE_OPWFMB_USED
            case EMIOS_PWM_IP_MODE_OPWFMB_FLAG:
            case EMIOS_PWM_IP_MODE_OPWFMB_FLAG_BOTH:
                ...
                Emios_Pwm_Ip_SetEdgePolarity(Base,
                                             Channel,
                                             (Polarity == EMIOS_PWM_IP_ACTIVE_HIGH) ? (Emios_Pwm_Ip_PolarityType)EMIOS_PWM_IP_ACTIVE_LOW : (Emios_Pwm_Ip_PolarityType)EMIOS_PWM_IP_ACTIVE_HIGH );
                break;
#endif
#ifdef EMIOS_PWM_IP_MODE_OPWFM_USED
            case EMIOS_PWM_IP_MODE_OPWFM_IMMEDIATE_UPDATE_FLAG:
...
*
* @implements Emios_Pwm_Ip_GetChannelMode_Activity
**/
...
*
* @implements Emios_Pwm_Ip_GetMasterBusChannel_Activity
**/
...
*
* @implements Emios_Pwm_Ip_SetPreEnableClock_Activity
**/
...
*
* @implements Emios_Pwm_Ip_SetBusSelected_Activity
**/
...
*
* @implements Emios_Pwm_Ip_SetClockPs_Activity
**/
...
*
* @implements Emios_Pwm_Ip_ComparatorTransferEnable_Activity
**/
...
*
* @implements Emios_Pwm_Ip_ComparatorTransferDisable_Activity
**/
...
*
* @implements Emios_Pwm_Ip_SyncUpdate_Activity
**/
