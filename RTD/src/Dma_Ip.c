...
#define DMA_IP_AR_RELEASE_MINOR_VERSION_C        9
#define DMA_IP_AR_RELEASE_REVISION_VERSION_C     0
#define DMA_IP_SW_MAJOR_VERSION_C                7
#define DMA_IP_SW_MINOR_VERSION_C                0
#define DMA_IP_SW_PATCH_VERSION_C                1
                                                                  ...
                                                                  )
{
    ...
    DMA_IP_DEV_ASSERT(NULL_PTR != Dma_Ip_pxInit->ppxLogicChannelConfigArray[LogicCh]);
    DMA_IP_DEV_ASSERT(Dma_Ip_pxInit->NumOfLogicChannel > LogicCh);
...
{
    const Dma_Ip_LogicChannelIdType*    LocLogicChId    = (const Dma_Ip_LogicChannelIdType *)&(Dma_Ip_pxInit->ppxLogicChannelConfigArray[paramLogicCh]->LogicChId);
          Dma_Ip_TcdRegType*            PxTcd           = NULL_PTR;
    ...
    else if (NULL_PTR != Dma_Ip_pxInit->ppxLogicChannelConfigArray[paramLogicCh]->ppxScatterGatherConfigArray)
    {
        ...
        HwAccDmaCh_SetControl_ScatterGatherAddress(LocHwVers, PxTcd, 0U);
        Dma_Ip_SetHwChannelState(LocHwVers, LocHwInst, LocHwCh, DMA_IP_CH_SCATTERGATHER_EVENT);
        LocStatus = Static_Dma_Ip_SetLogicChannelScatterGatherInit(LocLogicChId->LogicChId);
    }
    else
    {
...
Dma_Ip_ReturnType Dma_Ip_Init(const Dma_Ip_InitType * const DmaInit)
{
    ...
    for (LocCfgIdx = 0U; LocCfgIdx < Dma_Ip_pxInit->NumOfLogicInstance; LocCfgIdx++)
    {
    #if (STD_ON == DMA_IP_MULTIPARTITION_IS_AVAILABLE)
    ...
    for (LocCfgIdx = 0U; LocCfgIdx < Dma_Ip_pxInit->NumOfLogicChannel; LocCfgIdx++)
    {
    #if (STD_ON == DMA_IP_MULTIPARTITION_IS_AVAILABLE)
                                                ...
                                                )
{
    ...
    DMA_IP_DEV_ASSERT(NULL_PTR != Dma_Ip_pxInit);
    DMA_IP_DEV_ASSERT(Dma_Ip_pxInit->NumOfLogicInstance > LogicInst);
                                              ...
                                              )
{
    ...
    uint8 LocHwInst;
    uint8 LocHwCh;
    boolean LocChInstHrs; /* Stores the DMA instance / channel HRS bit value. */
#ifdef MCAL_ENABLE_USER_MODE_SUPPORT
#if (STD_ON == DMA_IP_USER_MODE_SUPPORT_IS_AVAILABLE)
#if (STD_ON == DMA_IP_REG_PROT_AVAILABLE)
#if (STD_ON == DMA_IP_TCD_REG_PROT_AVAILABLE)
    ...
    if (LocStatus == DMA_IP_STATUS_SUCCESS)
    {
        ...
        if (NULL_PTR != LocChState)
        {
            ...
            /* Save HRS flags. */
            HwAccDmaInst_GetHRSStatus(LocHwVers, LocHwInst, LocHwCh, &LocChInstHrs);
            ChStatus->HRS_present = LocChInstHrs;
        }
        else
        {
            /*Do nothing*/
...
static Dma_Ip_ReturnType Static_Dma_Ip_SetLogicChannelScatterGatherInit(const uint32 LogicCh)
{
    ...
    DMA_IP_DEV_ASSERT(NULL_PTR != Dma_Ip_pxInit);
    DMA_IP_DEV_ASSERT(NULL_PTR != Dma_Ip_pxInit->ppxLogicChannelConfigArray[LogicCh]);
    DMA_IP_DEV_ASSERT(NULL_PTR != Dma_Ip_pxInit->ppxLogicChannelConfigArray[LogicCh]->ppxScatterGatherConfigArray[LocElement]);

    LocLogicChId = (const Dma_Ip_LogicChannelIdType *)&(Dma_Ip_pxInit->ppxLogicChannelConfigArray[LogicCh]->LogicChId);
    LocScatterGather = Dma_Ip_pxInit->ppxLogicChannelConfigArray[LogicCh]->ppxScatterGatherConfigArray[LocElement];
    LocHwVers = LocLogicChId->HwVersId;
    LocHwInst = LocLogicChId->HwInstId;
    LocHwCh = LocLogicChId->HwChId;
#ifdef MCAL_ENABLE_USER_MODE_SUPPORT
