...
#define SIUL2_ICU_IP_AR_RELEASE_MINOR_VERSION_C    9
#define SIUL2_ICU_IP_AR_RELEASE_REVISION_VERSION_C 0
#define SIUL2_ICU_IP_SW_MAJOR_VERSION_C            7
#define SIUL2_ICU_IP_SW_MINOR_VERSION_C            0
#define SIUL2_ICU_IP_SW_PATCH_VERSION_C            1
...
==================================================================================================*/
    ...
    #if ((SIUL2_ICU_IP_AR_RELEASE_MAJOR_VERSION_C != SCHM_ICU_AR_RELEASE_MAJOR_VERSION) || \
        (SIUL2_ICU_IP_AR_RELEASE_MINOR_VERSION_C != SCHM_ICU_AR_RELEASE_MINOR_VERSION))
        #error "AutoSar Version Numbers of Siul2_Icu_Ip.c and SchM_Icu.h are different"
    #endif
...
/* Store dual clock mode configuration for each SIUL2 instance */
 ...
 * @details      Sets the (User Access Allowed) bit of the SIUL2 IP allowing SIUL2 registers writing in User Mode
 *
 * @param[in]    siul2BaseAddr  - SIUL2 instance address
 *
 * @return       void
 *
...
/**
 * @brief      Configure interrupt filter clock prescaler for SIUL2 instance
 * @details    Sets the interrupt filter clock prescaler value in IFCPR register for the specified
 *             SIUL2 instance to control digital filtering clock frequency.
 *
 * @param[in]  instance   - SIUL2 hardware instance number
 * @param[in]  prescaler  - Clock prescaler value for interrupt filter
 *
 * @return void
 */
...
#if(SIUL2_ICU_IP_VIRTWRAPPER_SUPPORT == STD_ON)
    /* Select base address based on virtual wrapper support */
    base = Siul2_Icu_Ip_apxBase[instance][Siul2_Icu_Ip_au8InterruptPdacSlot[instance]];
#else
    /* Use direct base address mapping */
    base = Siul2_Icu_Ip_apxBase[instance];
#endif

    /* Configure prescaler value for regular SIUL2 instance */
...
Siul2_Icu_Ip_StatusType Siul2_Icu_Ip_DeInit(uint8 instance)
{
...
#if(SIUL2_ICU_IP_VIRTWRAPPER_SUPPORT == STD_ON)
    /* Use virtual wrapper base address */
    base = Siul2_Icu_Ip_apxBase[instance][Siul2_Icu_Ip_au8InterruptPdacSlot[instance]];
#else
    /* Use direct base address mapping */
    base = Siul2_Icu_Ip_apxBase[instance];
    ...
    for(hwChannel = 0; hwChannel < (uint8)SIUL2_ICU_IP_NUM_OF_CHANNELS; hwChannel++)
    {
        ...
        /* Reset interrupt filter clock prescaler if deinitialization was successful */
...
#if(SIUL2_ICU_IP_VIRTWRAPPER_SUPPORT == STD_ON)
    /* Reset virtual wrapper PDAC slot */
    Siul2_Icu_Ip_au8InterruptPdacSlot[instance] = 0;
#endif

#if (SIUL2_ICU_IP_DUAL_CLOCK_MODE_API == STD_ON)
    /* Reset dual clock mode configuration */
...
Siul2_Icu_Ip_StatusType Siul2_Icu_Ip_Init(uint8 instance, const Siul2_Icu_Ip_ConfigType* userConfig)
{
...
#if(SIUL2_ICU_IP_VIRTWRAPPER_SUPPORT == STD_ON)
    /* Store virtual wrapper PDAC slot configuration */
    ...
    /* Configure global interrupt filter clock prescaler */
    Siul2_Icu_Ip_ConfigIntFilterClock(instance, userConfig->pInstanceConfig->intFilterClk);

#if (SIUL2_ICU_IP_DUAL_CLOCK_MODE_API == STD_ON)
    /* Store dual clock mode configuration */
    ...
    for(index=0; index < userConfig->numChannels; index++)
    {
        ...
        /* Calculate pin interrupt bit mask for this channel */
        pinIntValue = (1UL << hwChannel);
        /* Store callback information in channel state structure */
        ...
        if ((*userConfig->pChannelsConfig)[index].intEdgeSel != SIUL2_ICU_DISABLE)
        {
            ...
            /* Select external interrupt request for edge-triggered events */
            base->DIRSR0 &= ~pinIntValue;

            /* Clear any pending interrupt flags */
            base->DISR0  |= pinIntValue;
        }
        /* Initially mask interrupts for this channel (disable interrupt) */
...
/* Get base address for the instance */
#if(SIUL2_ICU_IP_VIRTWRAPPER_SUPPORT == STD_ON)
    base = Siul2_Icu_Ip_apxBase[instance][Siul2_Icu_Ip_au8InterruptPdacSlot[instance]];
#else
    base = Siul2_Icu_Ip_apxBase[instance];
    ...
    /* Read interrupt status flag and interrupt enable status for regular SIUL2 */
    flag = base->DISR0 & ((uint32)1U << hwChannel);
    irqEnable = base->DIRER0 & ((uint32)1U << hwChannel);

    /* Check if interrupt condition activated but interrupt not enabled */
