...
#define SIUL2_ICU_IRQ_AR_RELEASE_MINOR_VERSION_C       9
#define SIUL2_ICU_IRQ_AR_RELEASE_REVISION_VERSION_C    0
#define SIUL2_ICU_IRQ_SW_MAJOR_VERSION_C               7
#define SIUL2_ICU_IRQ_SW_MINOR_VERSION_C               0
#define SIUL2_ICU_IRQ_SW_PATCH_VERSION_C               1
...
/**
 * @brief      Processes group interrupt for SIUL2 ICU channels.
 * @details    Handles interrupt processing for a group of 8 consecutive SIUL2 ICU channels
 *             that share the same interrupt vector. Reads interrupt flags, clears them,
 *             and reports events for initialized channels with enabled interrupts.
 *
 * @param[in]  instance        - Hardware instance index of SIUL2 module.
 * @param[in]  firstHwChannel  - The first hardware channel index in the interrupt group.
 *
 * @return     void
 *
 */
static inline void Siul2_Icu_Ip_ProcessInterrupt(uint8 instance, uint8 firstHwChannel);
     ...
     (defined SIUL2_2_ICU_EIRQ_SINGLE_INT) || (defined SIUL2_3_ICU_EIRQ_SINGLE_INT) || \
     (defined SIUL2_4_ICU_EIRQ_SINGLE_INT) || (defined SIUL2_5_ICU_EIRQ_SINGLE_INT) || \
     (defined SIUL2_AE_ICU_EIRQ_SINGLE_INT))
/**
 * @brief      Driver function that reports ICU events to upper layer.
 * @details    This function handles event reporting by calling HLD callback functions
 *             or IPL notification functions based on the channel configuration. It provides
 *             the interface between hardware interrupt processing and application layer.
 *
 * @param[in]  instance   - Hardware instance index of SIUL2 module.
 * @param[in]  hwChannel  - Hardware channel index that triggered the event.
 *
 * @return     void
 */
...
/* Get SIUL2 base address */
    ...
    /* Loop through all channels in the instance */
    for (u8IrqChannel = (uint8)firstHwChannel; u8IrqChannel < (uint8)(firstHwChannel + 8U); u8IrqChannel++)
    {
        ...
        if (TRUE == Siul2_Icu_Ip_axChannelState[Siul2_Icu_Ip_au8IndexInChState[instance][u8IrqChannel]].chInit)
        {
            if (0x0U != (u32RegIrqMask & u32ChannelMask))
            {
                ...
                /* Call the event reporting function */
        ...
        /* Move to the next channel bit position */
