...
#define FLEXIO_UART_IP_AR_RELEASE_MINOR_VERSION_C       9
#define FLEXIO_UART_IP_AR_RELEASE_REVISION_VERSION_C    0
#define FLEXIO_UART_IP_SW_MAJOR_VERSION_C               7
#define FLEXIO_UART_IP_SW_MINOR_VERSION_C               0
#define FLEXIO_UART_IP_SW_PATCH_VERSION_C               1
                                      ...
                                      )
{
    ...
    FLEXIO_UART_IP_DEV_ASSERT(Bits >= 1U);
    FLEXIO_UART_IP_DEV_ASSERT(Divider >= 2U);
    ...
    Flexio_Uart_Ip_SetTimerCompare(Base, TIMER(UartUserCfg->Channel), (uint32)(((((uint32)Bits << 1U) - 1U) << 8U) + ((uint32)((uint32)Divider/2U) - 1U)));
...
static void Flexio_Uart_Ip_ReadData(const uint8 Channel)
{
    ...
    if (FLEXIO_UART_IP_8_BITS_PER_CHAR == UartUserCfg->BitCount)
    {
        *UartState->RxData = (uint8)(Data32b & (uint32)UINT8_MAX);
                                                    ...
                                                    )
{
    ...
    FLEXIO_UART_IP_DEV_ASSERT(UartState != NULL_PTR);
    FLEXIO_UART_IP_DEV_ASSERT(BitCountValue >= 1U);
    ...
    else
    {
        ...
        Flexio_Uart_Ip_SetTimerCompare(Base, TIMER(UartUserCfg->Channel), (uint32)(((((uint32)BitCountValue << 1U) - 1U) << 8U) + (uint32)CMP_value));
