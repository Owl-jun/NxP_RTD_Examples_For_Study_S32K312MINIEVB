...
#define PWM_IPW_AR_RELEASE_MINOR_VERSION_C      9
#define PWM_IPW_AR_RELEASE_REVISION_VERSION_C   0
#define PWM_IPW_SW_MAJOR_VERSION_C              7
...
==================================================================================================*/
#if (PWM_FLEXPWM_USED == STD_ON)
#if (PWM_FAULT_NOTIFICATION_SUPPORTED == STD_ON)
...
/* Array with initial channels fault interrupt enabled for each FlexPwm instance */
static uint8 Pwm_Ipw_aFlexpwmChsFaultInterruptEnabled[FLEXPWM_IP_INSTANCE_COUNT_U8];
...
#endif
#include "Pwm_MemMap.h"
#endif /* PWM_FAULT_NOTIFICATION_SUPPORTED == STD_ON */
...
static Etpu_Pwm_Ip_InstanceType Pwm_Ipw_aEtpuInstance[ETPU_PWM_IP_ENGINE_COUNT][ETPU_PWM_IP_CHANNEL_COUNT];
...
static inline void Pwm_Ipw_EmiosChannelModeCheck(Pwm_StateType State,
                                                 const Pwm_IpwChannelConfigType * const IpConfig
                                                );
...
#if (PWM_FLEXPWM_USED == STD_ON)
#if (PWM_SET_PERIOD_AND_DUTY_API == STD_ON)
static void Pwm_Ipw_FlexPwm_SetPeriodAndDuty(Pwm_Ipw_PeriodType              Period,
                                             uint16                          DutyCycle,
                                             const Pwm_IpwChannelConfigType * const IpConfig);
#endif /* (PWM_SET_PERIOD_AND_DUTY_API == STD_ON) */

#if ((PWM_UPDATE_DUTY_SYNCHRONOUS == STD_ON) && (PWM_SET_PERIOD_AND_DUTY_NO_UPDATE_API == STD_ON))
static void Pwm_Ipw_FlexPwm_SetPeriodAndDuty_NoUpdate(Pwm_Ipw_PeriodType              Period,
                                                      uint16                          DutyCycle,
                                                      const Pwm_IpwChannelConfigType * const IpConfig);
#endif /* (PWM_UPDATE_DUTY_SYNCHRONOUS == STD_ON) && (PWM_SET_PERIOD_AND_DUTY_NO_UPDATE_API == STD_ON) */

#if (PWM_SET_DUTY_CYCLE_API == STD_ON)
static void Pwm_Ipw_FlexPwm_SetDutyCycle(uint16                         DutyCycle,
                                         const Pwm_IpwChannelConfigType * const IpConfig);
#endif /* (PWM_SET_DUTY_CYCLE_API == STD_ON) */

#if (PWM_SET_DUTY_PHASE_SHIFT_API == STD_ON)
static void Pwm_Ipw_FlexPwm_SetDutyPhaseShift(uint16                           DutyCycle,
                                              Pwm_Ipw_DutyType                 PhaseShift,
                                              const Pwm_IpwChannelConfigType * const IpConfig,
                                              boolean                          SyncUpdate);
#endif /* (PWM_SET_DUTY_PHASE_SHIFT_API == STD_ON) */

#if (PWM_DE_INIT_API == STD_ON)
static void Pwm_Ipw_FlexPwm_DeInit(const Pwm_IpwChannelConfigType * const IpConfig, FlexPwm_Ip_OutputStateTypes FlexPwmChState);
...
#endif /* (PWM_GET_CHANNEL_STATE_API == STD_ON) || ((PWM_SET_CHANNEL_DEAD_TIME_API == STD_ON) && (PWM_DEV_ERROR_DETECT == STD_ON)) */
...
{
    uint8 InterruptMask;
    uint8 BaseRisingMask;
    uint8 BaseFallingMask;

    /* Determine base masks based on channel type */
    if (FLEXPWM_IP_PWMX == ChannelTypes)
    {
        BaseRisingMask = 0x01U;
        BaseFallingMask = 0x02U;
    }
    else if (FLEXPWM_IP_PWMA == ChannelTypes)
    {
        BaseRisingMask = 0x04U;
        BaseFallingMask = 0x08U;
    }
    else if (FLEXPWM_IP_PWMB == ChannelTypes)
    {
        BaseRisingMask = 0x10U;
        BaseFallingMask = 0x20U;
    }
    else
    {
        BaseRisingMask = 0x0U;
        BaseFallingMask = 0x0U;
    }

    /* Apply polarity and notification type logic */
    if (PWM_BOTH_EDGES == NotifTypes)
    {
        InterruptMask = BaseRisingMask | BaseFallingMask;
    }
    else if (((PWM_RISING_EDGE == NotifTypes) && (FLEXPWM_IP_POL_HIGH == Polarity)) ||
            ((PWM_FALLING_EDGE == NotifTypes) && (FLEXPWM_IP_POL_LOW == Polarity)))
    {
        InterruptMask = BaseRisingMask;
    }
    else
    {
        InterruptMask = BaseFallingMask;
    }
...
#if (PWM_FAULT_NOTIFICATION_SUPPORTED == STD_ON)
                /* Store initial channels fault interrupt enabled for each FlexPwm instance */
                if(TRUE == ((const FlexPwm_Ip_InstanceCfgTypes *)IpConfig->FlexPwmInsConfig)->FaultFunctionalityEnable)
                {
                    if(0x00U != ((const FlexPwm_Ip_InstanceCfgTypes *)IpConfig->FlexPwmInsConfig)->FaultChCfg->FaultInterruptEn)
                    {
                        Pwm_Ipw_aFlexpwmChsFaultInterruptEnabled[IpConfig->InstanceId] = ((const FlexPwm_Ip_InstanceCfgTypes *)IpConfig->FlexPwmInsConfig)->FaultChCfg->FaultInterruptEn;
                    }
                }
#endif
            }
...
void Pwm_Ipw_DeInit(const Pwm_IpwChannelConfigType * const IpConfig, Pwm_OutputStateType ChannelIdleState)
{
    ...
    Etpu_Pwm_Ip_InstanceType Etpu_InstanceDefaultSetting =
    {
        0U,
    ...
    Etpu_Pwm_Ip_ConfigType Etpu_ConfigDefaultSetting =
    {
        0U,
        0U,
    ...
    switch(IpConfig->ChannelType)
    {
        ...
        case PWM_CHANNEL_EMIOS:
            if (NULL_PTR != IpConfig->EmiosChConfig)
            {
                ...
                eMios_Pwm_Ip_aDeInitChannelCalledByASR[IpConfig->ChannelInstanceId] = TRUE;
#endif
                ...
                Emios_Pwm_Ip_SetOutputState(IpConfig->ChannelInstanceId,
                                           ...
                                           EmiosChState);
#ifdef EMIOS_PWM_IP_ASR_IPL_MIXED_CONTEXT
                eMios_Pwm_Ip_aDeInitChannelCalledByASR[IpConfig->ChannelInstanceId] = FALSE;
#endif
#if (PWM_GET_CHANNEL_STATE_API == STD_ON)
        ...
        case PWM_CHANNEL_FLEXIO:
            if (NULL_PTR != IpConfig->FlexioChConfig)
            {
#ifdef FLEXIO_PWM_IP_ASR_IPL_MIXED_CONTEXT
                Flexio_Pwm_Ip_aDeInitChannelCalledByASR[IpConfig->ChannelInstanceId] = TRUE;
#endif
...
#ifdef FLEXIO_PWM_IP_ASR_IPL_MIXED_CONTEXT
                Flexio_Pwm_Ip_aDeInitChannelCalledByASR[IpConfig->ChannelInstanceId] = FALSE;
#endif
        ...
        case PWM_CHANNEL_FLEXPWM:
            if (NULL_PTR != IpConfig->FlexPwmChConfig)
            {
                Pwm_Ipw_FlexPwm_DeInit(IpConfig, FlexPwmChState);
            }
            break;
#endif
#if (PWM_ETPU_USED == STD_ON)
        case PWM_CHANNEL_ETPU:
            if ((NULL_PTR != IpConfig->EtpuPwmInstance) && (NULL_PTR != IpConfig->EtpuChConfig))
            {
                ...
                Etpu_Pwm_Ip_DeInit(&Pwm_Ipw_aEtpuInstance[EtpuEngine][EtpuChannel],
                                   (PWM_HIGH == ChannelIdleState) ? (Etpu_Ip_PinStateType)ETPU_PIN_HIGH : (Etpu_Ip_PinStateType)ETPU_PIN_LOW );
...
void Pwm_Ipw_DeInitInstance(const Pwm_IpwInstanceConfigType * const IpConfig)
{
    ...
    switch(IpConfig->InstanceType)
    {
        ...
        case PWM_INSTANCE_FLEXPWM:
            if (NULL_PTR != IpConfig->FlexPwmInsConfig)
            {
#ifdef FLEXPWM_IP_ASR_IPL_MIXED_CONTEXT
                FlexPwm_Ip_aDeInitCalledByASR[IpConfig->InstanceId] = TRUE;
#endif
                /* Call FlexPwm IP de-initialize instances */
                FlexPwm_Ip_DeInit(IpConfig->InstanceId);
#ifdef FLEXPWM_IP_ASR_IPL_MIXED_CONTEXT
                FlexPwm_Ip_aDeInitCalledByASR[IpConfig->InstanceId] = FALSE;
#endif
            }
            break;
#endif
...
#if (PWM_FLEXPWM_USED == STD_ON)
        case PWM_CHANNEL_FLEXPWM:
            Pwm_Ipw_FlexPwm_SetDutyCycle (DutyCycle, IpConfig);
            break;
#endif
#if (PWM_ETPU_USED == STD_ON)
        case PWM_CHANNEL_ETPU:
...
#if (PWM_FLEXPWM_USED == STD_ON)
        case PWM_CHANNEL_FLEXPWM:
            Pwm_Ipw_FlexPwm_SetPeriodAndDuty(Period, DutyCycle, IpConfig);
            break;
#endif
#if (PWM_ETPU_USED == STD_ON)
        case PWM_CHANNEL_ETPU:
    ...
    switch(IpConfig->ChannelType)
    {
        ...
        case PWM_CHANNEL_ETPU:
            ...
            Etpu_Pwm_Ip_StopAndSetPin(&Pwm_Ipw_aEtpuInstance[EtpuEngine][EtpuChannel],
                                      (PWM_HIGH == IdleState) ? (Etpu_Ip_PinStateType)ETPU_PIN_HIGH : (Etpu_Ip_PinStateType)ETPU_PIN_LOW );
    ...
    switch(IpConfig->ChannelType)
    {
        ...
        case PWM_CHANNEL_EMIOS:
            ...
            if((EMIOS_PWM_IP_MODE_DAOC_FLAG != ((const Emios_Pwm_Ip_ChannelConfigType *)IpConfig->EmiosChConfig)->Mode) &&
               (EMIOS_PWM_IP_MODE_DAOC_FLAG_BOTH != ((const Emios_Pwm_Ip_ChannelConfigType *)IpConfig->EmiosChConfig)->Mode) &&
               (EMIOS_PWM_IP_INTERRUPT_REQUEST == ((const Emios_Pwm_Ip_ChannelConfigType *)IpConfig->EmiosChConfig)->IrqMode))
            {
...
/*===============================================================================================*/
#if (PWM_DMA_REQUEST_SUPPORTED == STD_ON)
/**
* @brief        Pwm_Ipw_DisableNotification
* @details      This function will call DisableDmaRequest function for the corresponding IP of PwmChannel
*
* @param[in]    IpConfig       Pointer to PWM top configuration structure
*
* @return       void
*
*/
void Pwm_Ipw_DisableDmaRequest(const Pwm_IpwChannelConfigType * const IpConfig)
{
    ...
    switch(IpConfig->ChannelType)
    {
        ...
        case PWM_CHANNEL_EMIOS:
            ...
            if((EMIOS_PWM_IP_MODE_DAOC_FLAG != ((const Emios_Pwm_Ip_ChannelConfigType *)IpConfig->EmiosChConfig)->Mode) &&
               (EMIOS_PWM_IP_MODE_DAOC_FLAG_BOTH != ((const Emios_Pwm_Ip_ChannelConfigType *)IpConfig->EmiosChConfig)->Mode) &&
               (EMIOS_PWM_IP_DMA_REQUEST == ((const Emios_Pwm_Ip_ChannelConfigType *)IpConfig->EmiosChConfig)->IrqMode))
            {
                Emios_Pwm_Ip_SetFlagRequest(IpConfig->ChannelInstanceId,
                            ((const Emios_Pwm_Ip_ChannelConfigType *)IpConfig->EmiosChConfig)->ChannelId,
                            EMIOS_PWM_IP_NOTIFICATION_DISABLED);
...
#endif /* PWM_DMA_REQUEST_SUPPORTED */

/*===============================================================================================*/
#if (PWM_DMA_REQUEST_SUPPORTED == STD_ON)
/**
* @brief        Pwm_Ipw_EnableNotification
* @details      This function will call EnableDmaRequest function for the corresponding IP of PwmChannel
*
* @param[in]    IpConfig       Pointer to PWM top configuration structure
*
* @return       void
*
*/
void Pwm_Ipw_EnableDmaRequest(const Pwm_IpwChannelConfigType * const IpConfig)
{
    switch(IpConfig->ChannelType)
    {
#if (PWM_EMIOS_USED == STD_ON)
                              ...
                              const Pwm_IpwChannelConfigType * const IpConfig)
{
    ...
    {
#if (PWM_EMIOS_USED == STD_ON)
        Pwm_Ipw_EmiosChannelModeCheck (State, IpConfig);
#endif
    }
    else
    {
...
#if (PWM_FLEXPWM_USED == STD_ON)
        case PWM_CHANNEL_FLEXPWM:
            Pwm_Ipw_FlexPwm_SetDutyPhaseShift(DutyCycle, PhaseShift, IpConfig, SyncUpdate);
            break;
#endif
        default:
            /* Nothing to do */
...
#if (PWM_FLEXPWM_USED == STD_ON)
        case PWM_CHANNEL_FLEXPWM:
            Pwm_Ipw_FlexPwm_SetPeriodAndDuty_NoUpdate (Period, DutyCycle, IpConfig);
            break;
#endif
        default:
            /* Nothing to do. */
                                            ...
                                            const Pwm_IpwChannelConfigType * const IpConfig)
{
    ...
    switch(IpConfig->ChannelType)
    {
        ...
        case PWM_CHANNEL_FLEXIO:
            /* Call Flexio IP function */
            if (FLEXIO_PWM_IP_IRQ_DISABLED == IpConfig->FlexioChConfig->IrqMode)
            {
                RetVal = (Std_ReturnType)E_NOT_OK;
            }
            else
...
/*===============================================================================================*/
#if ((PWM_DEV_ERROR_DETECT == STD_ON) && (PWM_FAULT_NOTIFICATION_SUPPORTED == STD_ON))
/**
* @brief        Pwm_Ipw_ValidateFaultInterruptSource.
* @details      This function will check the validity of the fault interrupt source requested when calling the Pwm_EnableFaultNotification/Pwm_DisableFaultNotification Apis.
*
* @param[in]    Pwm_FaultInterruptSourceType The fault interrupt source requested
*
* @return       Std_ReturnType
*               E_NOT_OK  The fault interrupt source is not valid.
*               E_OK      The fault interrupt source is valid.
*/
Std_ReturnType Pwm_Ipw_ValidateFaultInterruptSource(Pwm_FaultInterruptSourceType FaultInterruptSource)
{
    ...
    if (
        (PWM_FLEXPWM_0_FAULT_INPUT_0 == FaultInterruptSource) ||
        (PWM_FLEXPWM_0_FAULT_INPUT_1 == FaultInterruptSource) ||
        (PWM_FLEXPWM_0_FAULT_INPUT_2 == FaultInterruptSource) ||
        (PWM_FLEXPWM_0_FAULT_INPUT_3 == FaultInterruptSource) ||
        (PWM_FLEXPWM_1_FAULT_INPUT_0 == FaultInterruptSource) ||
        (PWM_FLEXPWM_1_FAULT_INPUT_1 == FaultInterruptSource) ||
        (PWM_FLEXPWM_1_FAULT_INPUT_2 == FaultInterruptSource) ||
        (PWM_FLEXPWM_1_FAULT_INPUT_3 == FaultInterruptSource)
       )
    {
        RetVal = (Std_ReturnType)E_OK;
    }
...
#endif /* (PWM_DEV_ERROR_DETECT == STD_ON) && (PWM_FAULT_NOTIFICATION_SUPPORTED == STD_ON) */
...
/*===============================================================================================*/
#if (PWM_DEV_ERROR_DETECT && ((PWM_NOTIFICATION_SUPPORTED == STD_ON) || (PWM_DMA_REQUEST_SUPPORTED == STD_ON)))
/**
* @brief        Pwm_Ipw_ValidateChannelFlagRequest
* @details      This function will check the validity of the hardware channel flag request for a given channel.
*
* @param[in]    IpConfig       Pointer to PWM top configuration structure
...
*
*/
Std_ReturnType Pwm_Ipw_ValidateChannelFlagRequest(const Pwm_IpwChannelConfigType * const IpConfig, uint8 ServiceId)
{
    /** @brief  Variable to store the value returned by the function */
    Std_ReturnType RetVal = (Std_ReturnType)E_NOT_OK;
#if (PWM_EMIOS_USED == STD_ON)
    Emios_Pwm_Ip_InterruptType InterruptType;
#endif

#if ((PWM_EMIOS_USED == STD_ON) || (PWM_FLEXIO_USED == STD_ON) || (PWM_FLEXPWM_USED == STD_ON) || (PWM_ETPU_USED == STD_ON))
    switch(IpConfig->ChannelType)
    {
#if (PWM_EMIOS_USED == STD_ON)
        case PWM_CHANNEL_EMIOS:
            InterruptType = IpConfig->EmiosChConfig->IrqMode;
            switch(ServiceId)
            {
                case PWM_DISABLENOTIFICATION_ID:
                case PWM_ENABLENOTIFICATION_ID:
                    if(EMIOS_PWM_IP_INTERRUPT_REQUEST== InterruptType)
                    {
                        RetVal = (Std_ReturnType)E_OK;
                    }
                    break;
                case PWM_DISABLEDMAREQUEST_ID:
                case PWM_ENABLEDMAREQUEST_ID:
                    if(EMIOS_PWM_IP_DMA_REQUEST== InterruptType)
                    {
                        RetVal = (Std_ReturnType)E_OK;
                    }
                    break;
...
#else
    (void) IpConfig;
#endif /* (PWM_EMIOS_USED == STD_ON) || (PWM_FLEXIO_USED == STD_ON) || (PWM_FLEXPWM_USED == STD_ON) || (PWM_ETPU_USED == STD_ON) */

    return RetVal;
}
#endif /* PWM_DEV_ERROR_DETECT && ((PWM_NOTIFICATION_SUPPORTED == STD_ON) || (PWM_DMA_REQUEST_SUPPORTED == STD_ON)) */
...
* @details      This function shall enable the PWM fault notification for the desired source
*
* @param[in]    FaultInterruptSource        The source of fault's desired to generate interrupt
*                                            Ex:  PWM_FLEXPWM_0_FAULT_INPUT_0
*                                                 PWM_FLEXPWM_0_FAULT_INPUT_1
*                                                             ....
*                                                 PWM_FLEXPWM_1_FAULT_INPUT_2
*                                                 PWM_FLEXPWM_1_FAULT_INPUT_3
*
* @return       void
*
*/
void Pwm_Ipw_EnableFaultNotification (Pwm_FaultInterruptSourceType FaultInterruptSource)
{
#if (PWM_FLEXPWM_USED == STD_ON)
    uint8 ModuleId         = 0U;
    uint8 MaskFaultInputId = 0U;
    uint8 InterruptMask    = 0U;

    ModuleId         = (uint8)FaultInterruptSource / FLEXPWM_IP_FAULT_CHANNELS_NO_U8;
    MaskFaultInputId = (uint8)1U << ((uint8)FaultInterruptSource % FLEXPWM_IP_FAULT_CHANNELS_NO_U8);
    InterruptMask    = Pwm_Ipw_aFlexpwmChsFaultInterruptEnabled[ModuleId] | MaskFaultInputId;
    /* Call FlexPwm IP function */
    FlexPwm_Ip_EnableInterrupt (ModuleId, 0U, FLEXPWM_IP_FAULT_INT, InterruptMask);
    Pwm_Ipw_aFlexpwmChsFaultInterruptEnabled[ModuleId] |= MaskFaultInputId;
#else
    /* This parameter is not used */
    (void)FaultInterruptSource;
#endif
}
#endif /* PWM_FAULT_NOTIFICATION_SUPPORTED */

/*===============================================================================================*/
#if (PWM_FAULT_NOTIFICATION_SUPPORTED == STD_ON)
/**
* @brief        Pwm_Ipw_DisableFaultNotification
* @details      This function shall disable the PWM fault notification for the desired source
*
* @param[in]    FaultInterruptSource        The source of fault's desired to generate interrupt
*                                            Ex:  PWM_FLEXPWM_0_FAULT_INPUT_0
*                                                 PWM_FLEXPWM_0_FAULT_INPUT_1
...
*
*/
void Pwm_Ipw_DisableFaultNotification (Pwm_FaultInterruptSourceType FaultInterruptSource)
{
#if (PWM_FLEXPWM_USED == STD_ON)
    uint8 ModuleId         = 0U;
    uint8 MaskFaultInputId = 0U;
    ...
    MaskFaultInputId = (uint8)1U << ((uint8)FaultInterruptSource % FLEXPWM_IP_FAULT_CHANNELS_NO_U8);
    /* Call FlexPwm IP function */
    FlexPwm_Ip_DisableInterrupt (ModuleId, 0U, FLEXPWM_IP_FAULT_INT, MaskFaultInputId);
    Pwm_Ipw_aFlexpwmChsFaultInterruptEnabled[ModuleId] &= ~MaskFaultInputId;
#else
    /* This parameter is not used */
    (void)FaultInterruptSource;
