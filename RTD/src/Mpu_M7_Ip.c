...
#define CDD_PLATFORM_MPU_M7_IP_AR_RELEASE_MINOR_VERSION_C       9
#define CDD_PLATFORM_MPU_M7_IP_AR_RELEASE_REVISION_VERSION_C    0
#define CDD_PLATFORM_MPU_M7_IP_SW_MAJOR_VERSION_C               7
#define CDD_PLATFORM_MPU_M7_IP_SW_MINOR_VERSION_C               0
#define CDD_PLATFORM_MPU_M7_IP_SW_PATCH_VERSION_C               1
...
uint32 Mpu_M7_Ip_GetErrorDetails_Privileged(Mpu_M7_Ip_ErrorDetailsType * pErrorDetails);
...
static void Mpu_M7_Ip_ComputeAccessRights(uint32 * pRegionAttr,
                                          Mpu_M7_Ip_AccessRightsType eAccessRights);
...
{
    uint8 u8Region;
    const Mpu_M7_Ip_RegionConfigType * pRegionCfg;
    S32_MPU_Type * pBase = S32_MPU;
    ...
    DevAssert(pConfig->u8RegionCnt <= Mpu_M7_Ip_GetDRegion(pBase));
#endif
    ...
    if (TRUE == pConfig->bDefaultMapEn)
    {
        pBase->CTRL |=S32_MPU_CTRL_PRIVDEFENA_MASK;
    }
    else
    {
        pBase->CTRL &= (~((uint32)S32_MPU_CTRL_PRIVDEFENA_MASK));
    }

    if (TRUE == pConfig->bEnableRunHFNMI)
    {
        pBase->CTRL |=S32_MPU_CTRL_HFNMIENA_MASK;
    }
    else
    {
        pBase->CTRL &= (~((uint32)S32_MPU_CTRL_HFNMIENA_MASK));
    }

    if (TRUE == pConfig->bEnMemManageInterrupt)
    {
    ...
    {
        pRegionCfg = &pConfig->pRegionConfigArr[u8Region];
        Mpu_M7_Ip_SetRegionConfig_Privileged((pConfig->pRegionConfigArr[u8Region]).u8RegionNum, pRegionCfg);
    }

    MCAL_DATA_SYNC_BARRIER();
    MCAL_INSTRUCTION_SYNC_BARRIER();

    pBase->CTRL |= S32_MPU_CTRL_ENABLE(1U);
...
{
    S32_MPU_Type * pBase = S32_MPU;
    uint32 u32RegionSize = 0UL;
    uint32 u32RegionAttributes = 0UL;
    uint32 u32Size;
#if (MPU_M7_IP_DEV_ERROR_DETECT == STD_ON)
    uint32 u32AlignmentMask;

    /* Check if the region size is at least the minimum supported */
    DevAssert(u8RegionNum < Mpu_M7_Ip_GetDRegion(pBase));
    DevAssert(pUserConfigPtr != NULL_PTR);
    DevAssert((pUserConfigPtr->u32EndAddr - pUserConfigPtr->u32StartAddr) >= (MPU_M7_IP_MIN_REGION_SIZE - 1U));
#endif

    /* Calculate log2(region) */
    u32Size = pUserConfigPtr->u32EndAddr - pUserConfigPtr->u32StartAddr;
    while ((u32Size > 0U) && (u32RegionSize < 32U))
    {
        u32RegionSize++;
        u32Size >>= 1U;
    }
...
#if (MPU_M7_IP_DEV_ERROR_DETECT == STD_ON)
    if (32U == u32RegionSize)
    {
        /* For 32-bit region size, check if address is 0 - fully aligned to 4GB boundary */
        u32AlignmentMask = 0xFFFFFFFFUL;
    }
    else
    {
        u32AlignmentMask = (1UL << u32RegionSize) - 1UL;
    }
    /* Check if the region start address is aligned to the region's size */
    DevAssert((pUserConfigPtr->u32StartAddr & u32AlignmentMask) == 0UL);
#endif
    SchM_Enter_Platform_PLATFORM_EXCLUSIVE_AREA_00();
    /* Set the region number */
    pBase->RNR = u8RegionNum;
    /* Set the region base address */
    pBase->RBAR = pUserConfigPtr->u32StartAddr;
    /* Set the region size */
    u32RegionAttributes |= S32_MPU_RASR_SIZE(u32RegionSize - 1UL);

    /* Configure sub region disable mask if the size of the region allows it */
    if ((pUserConfigPtr->u32EndAddr - pUserConfigPtr->u32StartAddr) >= (MPU_M7_IP_MIN_REGION_SIZE_WITH_SUBREGION - 1U))
    {
        u32RegionAttributes |= S32_MPU_RASR_SRD(pUserConfigPtr->u8SubRegMask);
    }

    /* Configure shareability */
    if (pUserConfigPtr->bShareable == TRUE)
    {
        u32RegionAttributes |=S32_MPU_RASR_S_MASK;
    }
    else
    {
        u32RegionAttributes &= (~((uint32)S32_MPU_RASR_S_MASK));
    }

    /* Set memory attributes and cacheability options */
    if (pUserConfigPtr->eMemType != MPU_M7_MEM_NORMAL_CACHEABLE)
    {
        Mpu_M7_Ip_SetMemoryType(&u32RegionAttributes, pUserConfigPtr->eMemType);
    }
    else
    {
        Mpu_M7_Ip_SetCachePolicies(&u32RegionAttributes, pUserConfigPtr->eInnerCachePolicy, pUserConfigPtr->eOuterCachePolicy);
    }

    /* Compute access rights */
    Mpu_M7_Ip_ComputeAccessRights(&u32RegionAttributes, pUserConfigPtr->eAccessRight);

    /* Enable Region */
    u32RegionAttributes |= S32_MPU_RASR_ENABLE(1UL);

    MCAL_DATA_SYNC_BARRIER();
    MCAL_INSTRUCTION_SYNC_BARRIER();

    /* Write final values */
    pBase->RASR = u32RegionAttributes;
...
void Mpu_M7_Ip_Deinit_Privileged(void)
{
    ...
    for (u8Region = 0U; u8Region < Mpu_M7_Ip_GetDRegion(base); u8Region++)
    {
        base->RNR = (uint32)u8Region;
        base->RASR = 0UL;
        base->RBAR = 0UL;
    }
}
...
void Mpu_M7_Ip_EnableRegion_Privileged(uint8 u8RegionNum, boolean bEnable)
{
    ...
    pBase->RNR = (uint32)u8RegionNum;
    if (bEnable == TRUE)
    {
        MCAL_DATA_SYNC_BARRIER();
        MCAL_INSTRUCTION_SYNC_BARRIER();

        pBase->RASR |= S32_MPU_RASR_ENABLE(1UL);
...
void Mpu_M7_Ip_SetAccessRight_Privileged(uint8 u8RegionNum, Mpu_M7_Ip_AccessRightsType eRights)
{
    ...
    pBase->RNR = (uint32)u8RegionNum;
    u32RegionAttributes = pBase->RASR;
    Mpu_M7_Ip_ComputeAccessRights(&u32RegionAttributes, eRights);
...
uint32 Mpu_M7_Ip_GetErrorDetails_Privileged(Mpu_M7_Ip_ErrorDetailsType * pErrorDetails)
{
    ...
    while ((u32Mmfsr != 0U) && (u8ErrorCount <= 1U))
    {
...
{
#ifdef MPU_M7_IP_ENABLE_USER_MODE_SUPPORT
    return (OsIf_Trusted_Call_Return1param(Mpu_M7_Ip_GetErrorDetails_Privileged, pErrorDetails) != 0UL) ? TRUE : FALSE;
#else
    return (Mpu_M7_Ip_GetErrorDetails_Privileged(pErrorDetails) != 0UL) ? TRUE : FALSE;
#endif
}
