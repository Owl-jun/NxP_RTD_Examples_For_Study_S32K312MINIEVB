...
#define EMIOS_MCL_IP_AR_RELEASE_MINOR_VERSION_C        9
#define EMIOS_MCL_IP_AR_RELEASE_REVISION_VERSION_C     0
#define EMIOS_MCL_IP_SW_MAJOR_VERSION_C                7
...
#if (EMIOS_IP_MULTIPARTITION_IS_AVAILABLE == STD_ON)
...
#if (STD_ON == EMIOS_IP_MULTIPARTITION_IS_AVAILABLE)
    Emios_Ip_axIpIsInitialized[Instance].runCore           = ConfigPtr->instanceCoreNumber;
    uint8 UserId = (uint8)OsIf_GetUserId();
    if (Emios_Ip_axIpIsInitialized[Instance].runCore == UserId)
    {
#endif /* STD_ON == EMIOS_IP_MULTIPARTITION_IS_AVAILABLE */
...
void Emios_Mcl_Ip_ComparatorTransferDisable(uint8 Instance, uint32 ChannelMask)
{
    ...
    /* Enter exclusive area to protect OUDIS */
    SchM_Enter_Mcl_MCL_EXCLUSIVE_AREA_47();
    Base->OUDIS |= ChannelMask;
    /* Exit exclusive area to protect OUDIS */
...
#if (STD_ON == EMIOS_IP_MULTIPARTITION_IS_AVAILABLE)
/** @implements Emios_Mcl_Ip_ValidateMultiPartitionInit_Activity */
boolean Emios_Mcl_Ip_ValidateMultiPartitionInit(uint8 HwInstance)
{
    boolean Valid = FALSE;
    uint8 UserId  = (uint8)OsIf_GetUserId();

    if (Emios_Ip_axIpIsInitialized[HwInstance].runCore == UserId)
    {
        Valid = TRUE;
    }
