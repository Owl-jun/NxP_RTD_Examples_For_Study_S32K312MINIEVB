...
#define GPT_AR_RELEASE_MINOR_VERSION_C       9

#define GPT_AR_RELEASE_REVISION_VERSION_C    0
#define GPT_SW_MAJOR_VERSION_C               7
...
#if ((GPT_VALIDATE_PARAM == STD_ON) && (GPT_MULTIPARTITION_ENABLED == STD_ON))
static inline Std_ReturnType Gpt_ValidateUserIdInit
(
    uint8 u8ServiceId,
    uint32 userId,
    const Gpt_ConfigType * configPtr
 ...
 *                 In case NULL_PTR == configPtr and (STD_OFF == GPT_PRECOMPILE_SUPPORT)
 *                 In case NULL_PTR != configPtr and (STD_ON == GPT_PRECOMPILE_SUPPORT)
 *                 The function will report an error to DET or SERR, depending on the environment the driver is run in.
...
#else /* STD_OFF == GPT_PRECOMPILE_SUPPORT */
        returnValue = (Std_ReturnType)E_OK;
#endif
    }
    else /* NULL_PTR == configPtr */
    {
#if (STD_ON == GPT_PRECOMPILE_SUPPORT)
        returnValue = (Std_ReturnType)E_OK;
#else /* STD_OFF == GPT_PRECOMPILE_SUPPORT */
...
#if ((GPT_VALIDATE_PARAM == STD_ON) && (GPT_MULTIPARTITION_ENABLED == STD_ON))
/**
 * @brief          This function validate the userId in initialization pointer.
 * @details        Validate the userId in initialization pointer.
 *                 In case NULL_PTR == configPtr and (STD_OFF == GPT_PRECOMPILE_SUPPORT)
 *                 In case NULL_PTR != configPtr and (STD_ON == GPT_PRECOMPILE_SUPPORT)
 *                 The function will report an error to DET or SERR, depending on the environment the driver is run in.
 * @param[in]       configPtr   Pointer to a selected configuration structure.
 ...
 *
 * @pre            The data structure including the configuration set required for initializing the GPT driver.
 * @implements     Gpt_ValidateUserIdInit_Activity
 *
 * */
static inline Std_ReturnType Gpt_ValidateUserIdInit
(
...
)
{
...
#if (STD_OFF == GPT_PRECOMPILE_SUPPORT)
...
#elif (STD_ON == GPT_PRECOMPILE_SUPPORT)
    if (GPT_MAX_PARTITIONS <= userId)
    {
        (void)Det_ReportError\
        (\
            (uint16)GPT_MODULE_ID,\
    ...
    else if (NULL_PTR == Gpt_Config[userId])
    {
        (void)Det_ReportError\
        (\
            (uint16)GPT_MODULE_ID,\
    ...
    else if (userId != Gpt_Config[userId]->u32PartitionId)
    {
        (void)Det_ReportError\
        (\
            (uint16)GPT_MODULE_ID,\
...
#endif /* (GPT_VALIDATE_PARAM == STD_ON) && (GPT_MULTIPARTITION_ENABLED == STD_ON) */
...
void Gpt_Init(const Gpt_ConfigType * configPtr)
{
    ...
    if((Std_ReturnType)E_OK == Gpt_ValidateGlobalCall(GPT_INIT_ID, userId))
    {
        ...
        if((Std_ReturnType)E_OK == Gpt_ValidateParamPtrInit(GPT_INIT_ID, configPtr))
        {
#endif
    ...
    #endif
#if ((GPT_VALIDATE_PARAM == STD_ON) && (GPT_MULTIPARTITION_ENABLED == STD_ON))
        /*Validate configuration userId in pointer parameter.*/
        if((Std_ReturnType)E_OK == Gpt_ValidateUserIdInit(GPT_INIT_ID, userId, configPtr))
        {
#endif
#if(GPT_PREDEFTIMER_FUNCTIONALITY_API == STD_ON)
