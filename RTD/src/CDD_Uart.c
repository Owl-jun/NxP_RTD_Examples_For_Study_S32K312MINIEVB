...
#define UART_AR_RELEASE_MINOR_VERSION_C       9
#define UART_AR_RELEASE_REVISION_VERSION_C    0
#define UART_SW_MAJOR_VERSION_C               7
#define UART_SW_MINOR_VERSION_C               0
#define UART_SW_PATCH_VERSION_C               1
...
void Uart_Init(const Uart_ConfigType * Config)
{
    ...
    #if (UART_MULTIPARTITION_SUPPORT == STD_ON)
    if (UART_MAX_PARTITIONS <= PartitionId)
    {
        #if (UART_DEV_ERROR_DETECT == STD_ON)
        /* Invalid PartitionId */
...
void Uart_Deinit(void)
{
    ...
    if (UART_MAX_PARTITIONS <= PartitionId)
    {
        #if (UART_DEV_ERROR_DETECT == STD_ON)
        /* Invalid PartitionId */
        (void)Det_ReportError((uint16)UART_MODULE_ID, (uint8)0, (uint8)UART_DEINIT_ID, (uint8)UART_E_PARAM_CONFIG);
        #endif /* (UART_DEV_ERROR_DETECT == STD_ON) */
    }
    else
    #endif /* (UART_MULTIPARTITION_SUPPORT == STD_ON) */
