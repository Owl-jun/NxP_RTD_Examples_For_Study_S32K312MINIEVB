...
#include "Lcu_Ip_Multipartition.h"
...
#define LCU_IP_AR_RELEASE_MINOR_VERSION_C        9
#define LCU_IP_AR_RELEASE_REVISION_VERSION_C     0
#define LCU_IP_SW_MAJOR_VERSION_C                7
#define LCU_IP_SW_MINOR_VERSION_C                0
#define LCU_IP_SW_PATCH_VERSION_C                1
...
#if (STD_ON == LCU_IP_MULTIPARTITION_IS_AVAILABLE)
/* Check if Lcu_Ip.c file and "Lcu_Ip_Multipartition.h" file are of the same vendor */
#if (LCU_IP_VENDOR_ID_C != LCU_IP_MULTIPARTITION_VENDOR_ID)
    #error "Lcu_Ip.c and "Lcu_Ip_Multipartition.h" have different vendor ids"
#endif

/* Check if Lcu_Ip.c file and "Lcu_Ip_Multipartition.h" file are of the same Autosar version */
#if ((LCU_IP_AR_RELEASE_MAJOR_VERSION_C != LCU_IP_MULTIPARTITION_AR_RELEASE_MAJOR_VERSION) || \
     (LCU_IP_AR_RELEASE_MINOR_VERSION_C != LCU_IP_MULTIPARTITION_AR_RELEASE_MINOR_VERSION) || \
     (LCU_IP_AR_RELEASE_REVISION_VERSION_C != LCU_IP_MULTIPARTITION_AR_RELEASE_REVISION_VERSION) \
    )
    #error "AutoSar Version Numbers of Lcu_Ip.c and "Lcu_Ip_Multipartition.h" are different"
#endif

/* Check if Lcu_Ip.c file and "Lcu_Ip_Multipartition.h" file are of the same Software version */
#if ((LCU_IP_SW_MAJOR_VERSION_C != LCU_IP_MULTIPARTITION_SW_MAJOR_VERSION) || \
     (LCU_IP_SW_MINOR_VERSION_C != LCU_IP_MULTIPARTITION_SW_MINOR_VERSION) || \
     (LCU_IP_SW_PATCH_VERSION_C != LCU_IP_MULTIPARTITION_SW_PATCH_VERSION) \
    )
    #error "Software Version Numbers of Lcu_Ip.c and "Lcu_Ip_Multipartition.h" are different"
#endif
#endif /* STD_ON == LCU_IP_MULTIPARTITION_IS_AVAILABLE */
...
Lcu_Ip_ReturnType Lcu_Ip_Init(const Lcu_Ip_InitType * const pxLcuInit)
{
    ...
    for (CfgIdx = 0U; CfgIdx < Lcu_Ip_pxInit->NumOfLogicInstance; CfgIdx++)
    {
        Lcu_Ip_LogicInstanceInit(Lcu_Ip_pxInit->ppxLogicInstanceConfigArray[CfgIdx]);
    }

    /* Load input configuration */
    for (CfgIdx = 0U; CfgIdx < Lcu_Ip_pxInit->NumOfLogicInput; CfgIdx++)
    {
        Lcu_Ip_LogicInputInit(CfgIdx, Lcu_Ip_pxInit->ppxLogicInputConfigArray[CfgIdx]->pxLcInputConfig);
    }

    /* Load output configuration */
    for (CfgIdx = 0U; CfgIdx < Lcu_Ip_pxInit->NumOfLogicOutput; CfgIdx++)
    {
...
static void Lcu_Ip_LogicInstanceInit(const Lcu_Ip_LogicInstanceType * pConfig)
{
    ...
    if (LocNumLogicCell > 0U)
    {
        for (CfgIdx = 0U; CfgIdx < LocNumLogicCell; CfgIdx++)
        {
            ...
            LocMaskValue = (ppxLocLogicCell->SwSynMode == TRUE) ? 1U : 0U;
            HwAcc_Lcu_SetSwSyncMode(HwInstance, HwLogicCell, LocMaskValue);
        }
    }
}
...
Lcu_Ip_ReturnType Lcu_Ip_SetSyncInputSwOverrideEnable(const Lcu_Ip_SyncInputValueType List[], const uint8 Dimension)
{
    ...
    for (LocIndex = 0U; LocIndex < LCU_INSTANCE_COUNT; LocIndex++)
    {
        MaskValue[LocIndex] = 0U;
        DataValue[LocIndex] = 0U;
    }
    ...
    for (LocIndex = 0U; LocIndex < LCU_INSTANCE_COUNT; LocIndex++)
    {
        if(0U != MaskValue[LocIndex])
        {
            RegValue = Lcu_Ip_paxBaseInst[LocIndex]->SWEN;
            RegValue &= ~MaskValue[LocIndex];
            RegValue |= DataValue[LocIndex];
            Lcu_Ip_paxBaseInst[LocIndex]->SWEN = RegValue;
        }
        else
        {
            /* Do nothing */
...
Lcu_Ip_ReturnType Lcu_Ip_SetSyncInputSwOverrideValue(const Lcu_Ip_SyncInputValueType List[], const uint8 Dimension)
{
    ...
    SchM_Enter_Mcl_MCL_EXCLUSIVE_AREA_23();
        for (LocIndex = 0U; LocIndex < LCU_INSTANCE_COUNT; LocIndex++)
        {
            if(0U != MaskValue[LocIndex])
            {
                RegValue = Lcu_Ip_paxBaseInst[LocIndex]->SWVALUE;
                RegValue &= ~MaskValue[LocIndex];
                RegValue |= DataValue[LocIndex];
                Lcu_Ip_paxBaseInst[LocIndex]->SWVALUE = RegValue;
            }
            else
            {
                /* Do nothing */
...
Lcu_Ip_ReturnType Lcu_Ip_SetSyncOutputDebugMode(const Lcu_Ip_SyncOutputValueType List[], const uint8 Dimension)
{
    ...
    SchM_Enter_Mcl_MCL_EXCLUSIVE_AREA_25();
        for (LocIndex = 0U; LocIndex < LCU_INSTANCE_COUNT; LocIndex++)
        {
            if(0U != MaskValue[LocIndex])
            {
                RegValue = Lcu_Ip_paxBaseInst[LocIndex]->DBGEN;
                RegValue &= ~MaskValue[LocIndex];
                RegValue |= DataValue[LocIndex];
                Lcu_Ip_paxBaseInst[LocIndex]->DBGEN = RegValue;
            }
            else
            {
                /* Do nothing */
...
Lcu_Ip_ReturnType Lcu_Ip_SetSyncOutputEnable(const Lcu_Ip_SyncOutputValueType List[], const uint8 Dimension)
{
    ...
    SchM_Enter_Mcl_MCL_EXCLUSIVE_AREA_38();
        for (LocIndex = 0U; LocIndex < LCU_INSTANCE_COUNT; LocIndex++)
        {
            if(0U != MaskValue[LocIndex])
            {
                RegValue = Lcu_Ip_paxBaseInst[LocIndex]->OUTEN;
                RegValue &= ~MaskValue[LocIndex];
                RegValue |= DataValue[LocIndex];
                Lcu_Ip_paxBaseInst[LocIndex]->OUTEN = RegValue;
            }
            else
            {
                /* Do nothing */
...
Lcu_Ip_ReturnType Lcu_Ip_GetSyncLogicInput(Lcu_Ip_SyncInputValueType List[], const uint8 Dimension)
{
    ...
    for (LocIndex = 0U; LocIndex < LCU_INSTANCE_COUNT; LocIndex++)
    {
        regLCU[LocIndex] = Lcu_Ip_paxBaseInst[LocIndex]->LCIN;
    }
#if (STD_ON == LCU_IP_MULTIPARTITION_IS_AVAILABLE)
    for (LocIndex = 0U; LocIndex < Dimension; LocIndex++)
    {
...
Lcu_Ip_ReturnType Lcu_Ip_GetSyncSwOverrideInput(Lcu_Ip_SyncInputValueType List[], const uint8 Dimension)
{
    ...
    for (LocIndex = 0U; LocIndex < LCU_INSTANCE_COUNT; LocIndex++)
    {
        regLCU[LocIndex] = Lcu_Ip_paxBaseInst[LocIndex]->SWOUT;
    }
#if (STD_ON == LCU_IP_MULTIPARTITION_IS_AVAILABLE)
    for (LocIndex = 0U; LocIndex < Dimension; LocIndex++)
    {
...
Lcu_Ip_ReturnType Lcu_Ip_GetSyncLogicOutput(Lcu_Ip_SyncOutputValueType List[], const uint8 Dimension)
{
    ...
    for (LocIndex = 0U; LocIndex < LCU_INSTANCE_COUNT; LocIndex++)
    {
        regLCU[LocIndex] = Lcu_Ip_paxBaseInst[LocIndex]->LCOUT;
    }
#if (STD_ON == LCU_IP_MULTIPARTITION_IS_AVAILABLE)
    for (LocIndex = 0U; LocIndex < Dimension; LocIndex++)
    {
...
Lcu_Ip_ReturnType Lcu_Ip_GetSyncForceOutput(Lcu_Ip_SyncOutputValueType List[], const uint8 Dimension)
{
    ...
    for (LocIndex = 0U; LocIndex < LCU_INSTANCE_COUNT; LocIndex++)
    {
        regLCU[LocIndex] = Lcu_Ip_paxBaseInst[LocIndex]->FORCEOUT;
    }
#if (STD_ON == LCU_IP_MULTIPARTITION_IS_AVAILABLE)
    for (LocIndex = 0U; LocIndex < Dimension; LocIndex++)
    {
...
Lcu_Ip_ReturnType Lcu_Ip_GetSyncForceStatus(Lcu_Ip_SyncOutputValueType List[], const uint8 Dimension)
{
    ...
    for (LocIndex = 0U; LocIndex < LCU_INSTANCE_COUNT; LocIndex++)
    {
        regLCU[LocIndex] = Lcu_Ip_paxBaseInst[LocIndex]->FORCESTS;
    }
#if (STD_ON == LCU_IP_MULTIPARTITION_IS_AVAILABLE)
    for (LocIndex = 0U; LocIndex < Dimension; LocIndex++)
    {
...
#if (STD_ON == LCU_IP_MULTIPARTITION_IS_AVAILABLE)
    LocStatus = Lcu_Ip_InputValidateCore(HwLcu, LocHwLcInputId);
    if (LocStatus == LCU_IP_STATUS_SUCCESS)
    {
#endif /* STD_ON == LCU_IP_MULTIPARTITION_IS_AVAILABLE */
...
static void Lcu_Ip_LogicOutputInit(const uint8 LogicOutput, const Lcu_Ip_LogicOutputConfigType * const pConfig)
{
...
#if (STD_ON == LCU_IP_MULTIPARTITION_IS_AVAILABLE)
    LocStatus = Lcu_Ip_OutputValidateCore(HwLcu, LocHwLcOutputId);
    if (LocStatus == LCU_IP_STATUS_SUCCESS)
    {
#endif /* STD_ON == LCU_IP_MULTIPARTITION_IS_AVAILABLE */
    (void)HwAcc_Lcu_AsyncSetOutputDebugMode(HwLcu, LocHwLcOutputId, (uint16)(pConfig->EnDebugMode ? 1U : 0U));
    ...
    (void)HwAcc_Lcu_AsyncSetOutputLutDma(HwLcu, LocHwLcOutputId, (uint16)(pConfig->EnLutDma ? 1U : 0U));
    (void)HwAcc_Lcu_AsyncSetOutputForceDma(HwLcu, LocHwLcOutputId, (uint16)(pConfig->EnForceDma ? 1U : 0U));
    (void)HwAcc_Lcu_AsyncSetOutputLutInt(HwLcu, LocHwLcOutputId, (uint16)(pConfig->EnLutInt ? 1U : 0U));
    (void)HwAcc_Lcu_AsyncSetOutputForceInt(HwLcu, LocHwLcOutputId, (uint16)(pConfig->EnForceInt ? 1U : 0U));
    (void)HwAcc_Lcu_AsyncSetOutputPolarity(HwLcu, LocHwLcOutputId, (uint16)(pConfig->InvertOutput ? 1U : 0U));
...
static void Lcu_Ip_LogicInstanceReset(uint8 LogicInstace)
{
    ...
    {
#if (STD_ON == LCU_IP_MULTIPARTITION_IS_AVAILABLE)
    LocStatus = Lcu_Ip_ValidateCore(HwInstance, LcIndex);
    if (LocStatus == LCU_IP_STATUS_SUCCESS)
    {
#endif /* STD_ON == LCU_IP_MULTIPARTITION_IS_AVAILABLE */
        ...
        HwAcc_Lcu_SetSwSyncMode(HwInstance, LcIndex, SW_SYNC_MODE_RESET_VALUE);
#if (STD_ON == LCU_IP_MULTIPARTITION_IS_AVAILABLE)
    }
#endif /* STD_ON == LCU_IP_MULTIPARTITION_IS_AVAILABLE */
    }
...
Lcu_Ip_ReturnType Lcu_Ip_SetAsyncInputList(const uint8 LogicInput, const Lcu_Ip_AsyncInputValueType List[], const uint8 Dimension)
{
   ...
   LCU_IP_DEV_ASSERT(Lcu_Ip_pxInit->NumOfLogicInput > LogicInput);
...
Lcu_Ip_ReturnType Lcu_Ip_SetAsyncOutputList(const uint8 LogicOutput, const Lcu_Ip_AsyncOutputValueType List[], const uint8 Dimension)
{
   ...
   LCU_IP_DEV_ASSERT(Lcu_Ip_pxInit->NumOfLogicOutput > LogicOutput);
