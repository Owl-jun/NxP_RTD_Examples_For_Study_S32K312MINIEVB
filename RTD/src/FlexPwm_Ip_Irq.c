...
#define FLEXPWM_IP_IRQ_AR_RELEASE_MINOR_VERSION_C       9
#define FLEXPWM_IP_IRQ_AR_RELEASE_REVISION_VERSION_C    0
#define FLEXPWM_IP_IRQ_SW_MAJOR_VERSION_C               7
#define FLEXPWM_IP_IRQ_SW_MINOR_VERSION_C               0
#define FLEXPWM_IP_IRQ_SW_PATCH_VERSION_C               1
...
#if (FLEXPWM_IP_IRQ_VENDOR_ID_C != FLEXPWM_IP_VENDOR_ID)
    #error "Vendor IDs of FlexPwm_Ip_Irq.c and FlexPwm_Ip.h are different."
#endif

/* Check if source file and FlexPwm_Ip.h file are of the same AUTOSAR version */
#if ((FLEXPWM_IP_IRQ_AR_RELEASE_MAJOR_VERSION_C    != FLEXPWM_IP_AR_RELEASE_MAJOR_VERSION) || \
     (FLEXPWM_IP_IRQ_AR_RELEASE_MINOR_VERSION_C    != FLEXPWM_IP_AR_RELEASE_MINOR_VERSION) || \
     (FLEXPWM_IP_IRQ_AR_RELEASE_REVISION_VERSION_C != FLEXPWM_IP_AR_RELEASE_REVISION_VERSION))
    #error "AUTOSAR version numbers of FlexPwm_Ip_Irq.c and FlexPwm_Ip.h are different."
#endif

/* Check if source file and FlexPwm_Ip.h file are of the same Software version */
#if ((FLEXPWM_IP_IRQ_SW_MAJOR_VERSION_C != FLEXPWM_IP_SW_MAJOR_VERSION) || \
     (FLEXPWM_IP_IRQ_SW_MINOR_VERSION_C != FLEXPWM_IP_SW_MINOR_VERSION) || \
     (FLEXPWM_IP_IRQ_SW_PATCH_VERSION_C != FLEXPWM_IP_SW_PATCH_VERSION))
    #error "Software version numbers of FlexPwm_Ip_Irq.c and FlexPwm_Ip.h are different."
#endif
...
static void FlexPwm_Ip_ProcessValidFaultIrq(uint8 ModuleIdx, uint8 Status);
#endif
...
void FlexPwm_Ip_ProcessCompareIrq(uint8 Instance, uint8 SubModule)
{
    ...
    else
    {
        ...
        if (0x00U != Status)
        {
            ...
            for (ChannelIdx = (uint8)0U; ChannelIdx < FLEXPWM_IP_NUMCHANNELS_PER_SUBMODULE_U8; ChannelIdx++)
            {
                UniqueSub = (uint8)((SubModule * FLEXPWM_IP_NUMCHANNELS_PER_SUBMODULE_U8) + ChannelIdx);
...
/**
 *
 * Function Name : FlexPwm_Ip_ProcessValidFaultIrq
 * Description   : Process a fault interrupt event when it is a valid
 *
 **/
static void FlexPwm_Ip_ProcessValidFaultIrq(uint8 ModuleIdx, uint8 Status)
{
    FlexPwm_Ip_HwAddrType *Base = FlexPwm_Ip_aBasePtr[ModuleIdx];
    uint8  FaultChannel;
    /* Clear interrupt flags */
    ...
    for (FaultChannel = (uint8)0U; FaultChannel < FLEXPWM_IP_FAULT_CHANNELS_NO_U8; FaultChannel++)
    {
...
*
* @return       void
* @implements FlexPwm_Ip_ProcessFaultIrq_Activity
*
* @implements   FlexPwm_Ip_ProcessFaultIrq_Activity
*
*/
void FlexPwm_Ip_ProcessFaultIrq(void)
{
    ...
    for (ModuleIdx = (uint8)0U; ModuleIdx < (uint8)FLEXPWM_IP_INSTANCE_COUNT_U8; ModuleIdx++)
    {
        ...
        else
        {
            ...
            if (0x00U != Status)
            {
                FlexPwm_Ip_ProcessValidFaultIrq(ModuleIdx, Status);
            }
        }
    }
}
