...
#define FLEXIO_PWM_IP_AR_RELEASE_MINOR_VERSION_C       9
#define FLEXIO_PWM_IP_AR_RELEASE_REVISION_VERSION_C    0
#define FLEXIO_PWM_IP_SW_MAJOR_VERSION_C               7
#define FLEXIO_PWM_IP_SW_MINOR_VERSION_C               0
#define FLEXIO_PWM_IP_SW_PATCH_VERSION_C               1
...
const Flexio_Pwm_Ip_ChannelConfigType * Flexio_Pwm_Ip_aState[FLEXIO_PWM_IP_INSTANCE_COUNT][FLEXIO_PWM_IP_CHANNEL_COUNT];
...
/* Arrays to used to check which layer Flexio_Pwm_Ip_DeInitChannel called in */
boolean Flexio_Pwm_Ip_aDeInitChannelCalledByASR[FLEXIO_PWM_IP_INSTANCE_COUNT]  = FLEXIO_PWM_IP_INITIAL_ARRAY;
...
static boolean Flexio_Pwm_Ip_PinOverrideNeeded(uint8 InstanceId, uint8 Channel, uint16 Period, uint16 DutyCycle)
{
    ...
    if ((0U == DutyCycle) || (0U == Period))
    {
        ...
        if (FLEXIO_PWM_IP_ACTIVE_HIGH == Flexio_Pwm_Ip_aState[InstanceId][Channel]->Polarity)
        {
#endif
            (void)Flexio_Pwm_Ip_ForceOuputLevel(InstanceId, Channel, FALSE);
#if (defined(FLEXIO_PWM_IP_HAS_LOW_MODE) && (FLEXIO_PWM_IP_HAS_LOW_MODE == STD_ON))
        }
        else
        {
            (void)Flexio_Pwm_Ip_ForceOuputLevel(InstanceId, Channel, TRUE);
        }
#endif
    }
...
{
    Flexio_Pwm_Ip_HwAddrType * const Base = Flexio_Pwm_Ip_aBasePtr[InstanceId];
    const Flexio_Pwm_Ip_ChannelConfigType * const UserCfg = Flexio_Pwm_Ip_aState[InstanceId][Channel];
...
Flexio_Pwm_Ip_StatusType Flexio_Pwm_Ip_InitChannel(uint8 InstanceId, const Flexio_Pwm_Ip_ChannelConfigType * const UserCfg)
{
    ...
    Flexio_Pwm_Ip_aState[InstanceId][UserCfg->TimerId] = UserCfg;
...
Flexio_Pwm_Ip_StatusType Flexio_Pwm_Ip_DeInitChannel(uint8 InstanceId, uint8 Channel)
{
    ...
    /* Reset the Pin configuration */
#ifdef FLEXIO_PWM_IP_ASR_IPL_MIXED_CONTEXT
    if (TRUE == Flexio_Pwm_Ip_aDeInitChannelCalledByASR[InstanceId])
    {
        Flexio_Pwm_Ip_ConfigurePinOverride(Base, UserCfg->PinId, FALSE);
        Flexio_Pwm_Ip_SetPinLevel(Base, UserCfg->PinId, FALSE);
    }
        ...
        Flexio_Pwm_Ip_ConfigurePinOverride(Base, UserCfg->PinId, FALSE);
        Flexio_Pwm_Ip_SetPinLevel(Base, UserCfg->PinId, FALSE);
    #endif /* defined(FLEXIO_PWM_IP_AUTOSAR_MODE_IS_USED) && (STD_OFF == FLEXIO_PWM_IP_AUTOSAR_MODE_IS_USED) */
#endif /* FLEXIO_PWM_IP_ASR_IPL_MIXED_CONTEXT */
#endif /* defined(FLEXIO_PWM_IP_HAS_PIN_OVERRIDE) && (FLEXIO_PWM_IP_HAS_PIN_OVERRIDE == STD_ON) */
    return RetStatus;
}
#if (defined(FLEXIO_PWM_IP_HAS_PRESCALER) && (FLEXIO_PWM_IP_HAS_PRESCALER == STD_ON))
