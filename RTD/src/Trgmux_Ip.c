...
#define TRGMUX_IP_AR_RELEASE_MINOR_VERSION_C        9
#define TRGMUX_IP_AR_RELEASE_REVISION_VERSION_C     0
#define TRGMUX_IP_SW_MAJOR_VERSION_C                7
...
Trgmux_Ip_StatusType Trgmux_Ip_Init(const Trgmux_Ip_InitType * const pxTrgmuxInit)
{
    ...
    for(LocInstanceIdx = 0U; LocInstanceIdx < pxTrgmuxInit->NumOfLogicInstance; LocInstanceIdx++)
    {
        LocHwInst = pxTrgmuxInit->paxLogicInstance[LocInstanceIdx];
        /* Clear register */
    ...
    if(TRGMUX_IP_STATUS_SUCCESS == Status)
    {
        ...
        for(LocTriggerIdx = 0U; LocTriggerIdx < pxTrgmuxInit->NumOfLogicTriger; LocTriggerIdx++)
        {
