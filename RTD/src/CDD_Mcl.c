...
#define CDD_MCL_AR_RELEASE_MINOR_VERSION_C        9
#define CDD_MCL_AR_RELEASE_REVISION_VERSION_C     0
#define CDD_MCL_SW_MAJOR_VERSION_C                7
...
static Mcl_ReturnType Mcl_IsValidConfigPtr(const void* ptr);

#if (STD_ON == MCL_DMA_IS_AVAILABLE)
static void Mcl_HandleDmaDeinit(uint32 userID);
#endif /* #if (STD_ON == MCL_DMA_IS_AVAILABLE) */

#if (STD_ON == MCL_LCU_IS_AVAILABLE)
static void Mcl_HandleLcuDeinit(uint32 userID);
#endif /* #if (STD_ON == MCL_LCU_IS_AVAILABLE) */

#if (STD_ON == MCL_EMIOS_IS_AVAILABLE)
static void Mcl_HandleEmiosDeinit(uint32 userID);
#endif /* #if (STD_ON == MCL_EMIOS_IS_AVAILABLE) */

#if (STD_ON == MCL_FLEXIO_IS_AVAILABLE)
static void Mcl_HandleFlexioDeinit(uint32 userID);
#endif /* #if (STD_ON == MCL_FLEXIO_IS_AVAILABLE) */
...
static Mcl_ReturnType Mcl_IsValidConfigPtr(const void* ptr)
{
    return (NULL_PTR != ptr) ? MCL_STATUS_SUCCESS : MCL_STATUS_ERROR;
}
...
static void Mcl_HandleDmaDeinit(uint32 userID)
{
    ...
    /* Check the Config Pointer*/
    Mcl_ReturnType status = Mcl_IsValidConfigPtr(
        (MCL_MAX_PARTITIONS > userID) ?
        Mcl_pxConfig[userID]->pxDmaInitType :
        Mcl_pxConfig[0U]->pxDmaInitType);

    if (MCL_STATUS_ERROR != status)
    {
        /* DeInit DMA IP*/
...
static void Mcl_HandleLcuDeinit(uint32 userID)
{
    ...
    Mcl_ReturnType status = Mcl_IsValidConfigPtr(
        (MCL_MAX_PARTITIONS > userID) ?
        Mcl_pxConfig[userID]->pxLcuInitType :
        Mcl_pxConfig[0U]->pxLcuInitType);

    if (MCL_STATUS_ERROR != status)
    {
#if (STD_ON == MCL_DET_IS_AVAILABLE)
        /* DeInit LCU IP*/
        Lcu_Ip_ReturnType ret = Lcu_Ip_Deinit();
        if (LCU_IP_STATUS_SUCCESS != ret)
        {
...
static void Mcl_HandleEmiosDeinit(uint32 userID)
{
    ...
    Mcl_ReturnType status = Mcl_IsValidConfigPtr(
        (MCL_MAX_PARTITIONS > userID) ?
        Mcl_pxConfig[userID]->pEmiosInitType :
        Mcl_pxConfig[0U]->pEmiosInitType);
...
static void Mcl_HandleFlexioDeinit(uint32 userID)
{
    ...
    Mcl_ReturnType status = Mcl_IsValidConfigPtr(
        (MCL_MAX_PARTITIONS > userID) ?
        Mcl_pxConfig[userID]->pFlexioInitType :
        Mcl_pxConfig[0U]->pFlexioInitType);

    if (MCL_STATUS_ERROR != status)
    {
        /* DeInit FLEXIO IP */
        if (MCL_MAX_PARTITIONS > userID)
        {
            Flexio_Mcl_Ip_DeinitDevice(Mcl_pxConfig[userID]->pFlexioInitType);
        }
        else
        {
            Flexio_Mcl_Ip_DeinitDevice(Mcl_pxConfig[0U]->pFlexioInitType);
        }
    }
}
#endif
...
#endif /* STD_ON == MCL_DET_IS_AVAILABLE */
    uint32 MclUserID;
    Mcl_ReturnType ValidPtrConfig;

    MclUserID = Mcl_GetUserID;

    if(MCL_MAX_PARTITIONS > MclUserID)
    {
        if((NULL_PTR == Mcl_pxConfig[MclUserID]))
        {
            ValidPtrConfig = MCL_STATUS_SUCCESS;
        }
        else
        {
            ValidPtrConfig = MCL_STATUS_ERROR;
        }
    }
    else
    {
        if((NULL_PTR == Mcl_pxConfig[0U]))
        {
            ValidPtrConfig = MCL_STATUS_SUCCESS;
        }
        else
...
#if (STD_ON == MCL_DET_IS_AVAILABLE)
    if ((MCL_STATUS_SUCCESS == ValidPtr) && (MCL_STATUS_SUCCESS == ValidPtrConfig))
#else /* STD_ON == MCL_DET_IS_AVAILABLE */
    if (MCL_STATUS_SUCCESS == ValidPtrConfig)
#endif /* STD_ON == MCL_DET_IS_AVAILABLE */
    {
...
{
    uint32 MclUserID = Mcl_GetUserID;
    Mcl_ReturnType validConfig;

    /* Check the ConfigPtr*/
    const Mcl_ConfigType *configPtr = (MCL_MAX_PARTITIONS > MclUserID) ?
                                      Mcl_pxConfig[MclUserID] :
                                      Mcl_pxConfig[0U];

    validConfig = Mcl_IsValidConfigPtr(configPtr);

    if (MCL_STATUS_SUCCESS == validConfig)
    {
    #if (STD_ON == MCL_DMA_IS_AVAILABLE)
        /* DeInit DMA IP*/
        Mcl_HandleDmaDeinit(MclUserID);
    #endif /* #if (STD_ON == MCL_DMA_IS_AVAILABLE) */
    #if (STD_ON == MCL_LCU_IS_AVAILABLE)
        /* DeInit LCU IP*/
        Mcl_HandleLcuDeinit(MclUserID);
    #endif /* #if (STD_ON == MCL_LCU_IS_AVAILABLE) */
    #if (STD_ON == MCL_EMIOS_IS_AVAILABLE)
        /* DeInit EMIOS IP*/
        Mcl_HandleEmiosDeinit(MclUserID);
    #endif /* #if (STD_ON == MCL_EMIOS_IS_AVAILABLE) */
    #if (STD_ON == MCL_FLEXIO_IS_AVAILABLE)
        /* DeInit FLEXIO IP*/
        Mcl_HandleFlexioDeinit(MclUserID);
    #endif /* #if (STD_ON == MCL_FLEXIO_IS_AVAILABLE) */
                              ...
                              )
{
    ...
    if (DMA_IP_DMA_TOTAL_NOF_LOGIC_INST <= Instance)
    {
                             ...
                             )
{
    ...
    if (DMA_IP_DMA_TOTAL_NOF_LOGIC_CHANNEL <= Channel)
    {
...
{
    Dma_Ip_ReturnType RetStatus;
    Dma_Ip_LogicChannelStatusType LocChStatus = {DMA_IP_CH_RESET_STATE, 0U, FALSE, FALSE, FALSE};
    boolean ChannelValidation = TRUE;
    boolean StatusValidation = TRUE;
    ...
    if ((FALSE != ChannelValidation) && (FALSE != StatusValidation))
    {
        ...
        Status->HRS_present  = LocChStatus.HRS_present;
...
void Mcl_Emios_SetReloadInterval(uint16 Channel,
                                 uint8 Interval
                                )
{
    ...
    {
#if (STD_ON == MCL_MULTIPARTITION_IS_AVAILABLE)
        if (TRUE == Emios_Mcl_Ip_ValidateMultiPartitionInit(HwInstance))
        {
#endif
...
void Mcl_Emios_SetCounterBusPeriod(uint16 logicalChannel,
                                   uint32 period,
                                   boolean syncUpdate
                                  )
{
...
#if (STD_ON == MCL_DET_IS_AVAILABLE)
#if (STD_ON == MCL_MULTIPARTITION_IS_AVAILABLE)
    if (Emios_Mcl_Ip_ValidateMultiPartitionInit(HwInstance))
    {
#endif /* STD_ON == MCL_MULTIPARTITION_IS_AVAILABLE */
...
void Mcl_Emios_SetClockMode(const Mcl_EmiosInitConfigType *const pConfig,
                            Mcl_EmiosSelectPrescalerType Prescaler
                           )
{
    uint8 CurrentInstance;
...
void Mcl_SetLcuSyncInputSwOverrideEnable(const Mcl_LcuSyncInputValueType List[],
                                        ...
                                        )
{
...
#endif /* MCL_MULTIPARTITION_IS_AVAILABLE */
    uint8 NumOfLogicInput;
    uint32 MclUserID;
    ...
    if(MCL_MAX_PARTITIONS > MclUserID)
    {
        NumOfLogicInput = Mcl_pxConfig[MclUserID]->pxLcuInitType->NumOfLogicInput;
    }
    else
    {
        NumOfLogicInput = Mcl_pxConfig[0U]->pxLcuInitType->NumOfLogicInput;
    }

    for (Index = 0; Index < Dimension; Index++)
    {
        if (List[Index].LogicInputId >= NumOfLogicInput)
        {
...
#else /* MCL_MULTIPARTITION_IS_AVAILABLE */
...
void Mcl_SetLcuSyncInputSwOverrideValue(const Mcl_LcuSyncInputValueType List[],
                                        const uint8 Dimension
                                       )
{
#if (STD_ON == MCL_DET_IS_AVAILABLE)
...
void Mcl_SetLcuSyncInputMuxSelect(const Mcl_LcuSyncInputValueType List[],
                                  const uint8 Dimension
                                 )
{
#if (STD_ON == MCL_DET_IS_AVAILABLE)
...
void Mcl_SetLcuSyncOutputDebugMode(const Mcl_LcuSyncOutputValueType List[],
                                   const uint8 Dimension
                                  )
{
#if (STD_ON == MCL_DET_IS_AVAILABLE)
    ...
    if(MCL_MAX_PARTITIONS > MclUserID)
    {
        NumOfLogicOutput = Mcl_pxConfig[MclUserID]->pxLcuInitType->NumOfLogicOutput;
    }
    else
    {
        NumOfLogicOutput = Mcl_pxConfig[0U]->pxLcuInitType->NumOfLogicOutput;
    }

    for (Index = 0; Index < Dimension; Index++)
    {
        if (List[Index].LogicOutputId >= NumOfLogicOutput)
        {
...
void Mcl_SetLcuSyncOutputEnable(const Mcl_LcuSyncOutputValueType List[],
                                const uint8 Dimension
                               )
{
#if (STD_ON == MCL_DET_IS_AVAILABLE)
...
void Mcl_SetLcuSyncOutputForceInputSensitivity(const Mcl_LcuSyncOutputValueType List[],
                                               const uint8 Dimension
                                              )
{
#if (STD_ON == MCL_DET_IS_AVAILABLE)
...
void Mcl_SetLcuSyncOutputForceClearingMode(const Mcl_LcuSyncOutputValueType List[],
                                           const uint8 Dimension
                                          )
{
#if (STD_ON == MCL_DET_IS_AVAILABLE)
...
void Mcl_SetLcuSyncOutputForceSyncSelect(const Mcl_LcuSyncOutputValueType List[],
                                         const uint8 Dimension
                                        )
{
#if (STD_ON == MCL_DET_IS_AVAILABLE)
...
void Mcl_SetLcuSyncOutputPolarity(const Mcl_LcuSyncOutputValueType List[],
                                  const uint8 Dimension
                                 )
{
#if (STD_ON == MCL_DET_IS_AVAILABLE)
...
void Mcl_SetLcuSyncOutputForceDma(const Mcl_LcuSyncOutputValueType List[],
                                  const uint8 Dimension
                                 )
{
#if (STD_ON == MCL_DET_IS_AVAILABLE)
...
void Mcl_SetLcuSyncOutputForceInt(const Mcl_LcuSyncOutputValueType List[],
                                  const uint8 Dimension
                                 )
{
#if (STD_ON == MCL_DET_IS_AVAILABLE)
...
#if (STD_ON == MCL_MULTIPARTITION_IS_AVAILABLE)
        if (LCU_IP_STATUS_WRONG_PARTITION == LocStatus)
        {
...
void Mcl_SetLcuSyncOutputLutDma(const Mcl_LcuSyncOutputValueType List[],
                                const uint8 Dimension
                               )
{
#if (STD_ON == MCL_DET_IS_AVAILABLE)
...
void Mcl_SetLcuSyncOutputLutInt(const Mcl_LcuSyncOutputValueType List[],
                                const uint8 Dimension
                               )
{
#if (STD_ON == MCL_DET_IS_AVAILABLE)
...
void Mcl_SetLcuSyncOutputFallFilter(const Mcl_LcuSyncOutputValueType List[],
                                    const uint8 Dimension
                                   )
{
#if (STD_ON == MCL_DET_IS_AVAILABLE)
...
void Mcl_SetLcuSyncOutputRiseFilter(const Mcl_LcuSyncOutputValueType List[],
                                    const uint8 Dimension
                                   )
{
#if (STD_ON == MCL_DET_IS_AVAILABLE)
...
void Mcl_SetLcuSyncOutputLutControl(const Mcl_LcuSyncOutputValueType List[],
                                    const uint8 Dimension
                                   )
{
#if (STD_ON == MCL_DET_IS_AVAILABLE)
...
void Mcl_ClearLcuSyncOutputForceEvent
(
    const Mcl_LcuSyncOutputValueType List[],
    const uint8 Dimension
)
{
#if (STD_ON == MCL_DET_IS_AVAILABLE)
...
void Mcl_GetLcuSyncLogicInput(Mcl_LcuSyncInputValueType List[],
                              const uint8 Dimension
                             )
{
#if (STD_ON == MCL_DET_IS_AVAILABLE)
...
void Mcl_GetLcuSyncSwOverrideInput(Mcl_LcuSyncInputValueType List[],
                                   const uint8 Dimension
                                  )
{
#if (STD_ON == MCL_DET_IS_AVAILABLE)
...
void Mcl_GetLcuSyncLogicOutput(Mcl_LcuSyncOutputValueType List[],
                               const uint8 Dimension
                              )
{
#if (STD_ON == MCL_DET_IS_AVAILABLE)
...
void Mcl_GetLcuSyncForceOutput(Mcl_LcuSyncOutputValueType List[],
                               const uint8 Dimension
                              )
{
#if (STD_ON == MCL_DET_IS_AVAILABLE)
...
void Mcl_GetLcuSyncForceStatus(Mcl_LcuSyncOutputValueType List[],
                               const uint8 Dimension
                              )
{
#if (STD_ON == MCL_DET_IS_AVAILABLE)
...
void Mcl_GetLcuSyncCombineForceInput(Mcl_LcuSyncOutputValueType List[],
                                     const uint8 Dimension
                                    )
{
#if (STD_ON == MCL_DET_IS_AVAILABLE)
...
void Mcl_SetLcuAsyncInputList(const uint8 LogicInput,
...
void Mcl_SetLcuAsyncOutputList(const uint8 LogicOutput,
                               const Mcl_LcuAsyncOutputValueType List[],
                               const uint8 Dimension
                              )
{
#if (STD_ON == MCL_DET_IS_AVAILABLE)
    ...
    if (NumOfLogicOutput <= LogicOutput)
    {
...
void Mcl_GetLcuAsyncLogicInputInfo(const uint8 LogicInput,
                                   const Mcl_LcuInputInfoParamType Param,
                                   uint8 * const Value
                                  )
{
...
void Mcl_GetLcuAsyncLogicOutputInfo(const uint8 LogicOutput,
                                    const Mcl_LcuOutputInfoParamType Param,
                                    uint8 * const Value
                                   )
{
