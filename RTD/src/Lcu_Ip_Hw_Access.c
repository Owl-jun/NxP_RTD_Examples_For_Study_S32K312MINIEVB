...
#define LCU_IP_HW_ACCESS_AR_RELEASE_MINOR_VERSION_C        9
#define LCU_IP_HW_ACCESS_AR_RELEASE_REVISION_VERSION_C     0
#define LCU_IP_HW_ACCESS_SW_MAJOR_VERSION_C                7
#define LCU_IP_HW_ACCESS_SW_MINOR_VERSION_C                0
#define LCU_IP_HW_ACCESS_SW_PATCH_VERSION_C                1
...
boolean HwAcc_Lcu_AsyncSetOutputForceInt(const uint8 LcuId, const uint8 HwLcOutputId, uint16 MaskValue)
{
    ...
    {
        /* Clear Force Status if enabling interrupt*/
        PtBase->FORCESTS = LCU_FORCESTS_FORCESTS((uint32)MaskValue << HwLcOutputId);
        /* Enable or disable Force interrupt*/
...
boolean HwAcc_Lcu_AsyncSetOutputLutInt(const uint8 LcuId, const uint8 HwLcOutputId, uint16 MaskValue)
{
    ...
    {
        /* Clear Lut Status if enabling interrupt*/
        PtBase->LC[LcId].STS = LCU_STS_LUT_STS((uint32)MaskValue << HwOutput);
        /* Enable or disable LUT interrupt*/
        Reg &= ~(uint32)((uint32)1U << HwOutput);
...
void HwAcc_Lcu_SetSwSyncMode(const uint8 LcuId, const uint8 LcId, uint8 MaskValue)
{
    LCU_Type * PtBase = Lcu_Ip_paxBaseInst[LcuId];
    uint32 Reg =  PtBase->LC[LcId].SCTRL;

    /* Register access */
    Reg &= ~(LCU_SCTRL_SW_MODE_MASK);
    Reg |= LCU_SCTRL_SW_MODE(MaskValue);
    PtBase->LC[LcId].SCTRL = Reg;
}
