...
#define WDG_CHANNEL_AR_RELEASE_MINOR_VERSION_C       9
#define WDG_CHANNEL_AR_RELEASE_REVISION_VERSION_C    0
#define WDG_CHANNEL_SW_MAJOR_VERSION_C               7
                                                 ...
                                                 const Wdg_Ipw_InstanceType Instance)
{
    ...
    if ((Timeout > CfgMaxTimeout) || (((uint32)Timeout <= Wdg_au32WdgTimeOutPeriod[Instance][Wdg_aeCurrentMode[Instance]]) && (Timeout != (uint16)0U)))
    {
...
void  Wdg_ChannelClearResetRequest(const Wdg_Ipw_InstanceType Instance)
{
    ...
    UserID = (uint32) Wdg_GetUserID();
    Valid = Wdg_ChannelValidatePartitionUsed(UserID, Instance,WDG_CLEARRESETREQUEST_ID,WDG_E_PARAM_CONFIG);
    if ((Std_ReturnType)E_OK == Valid)
#endif /* (WDG_MULTIPARTITION_ENABLED == STD_ON) */
    {
