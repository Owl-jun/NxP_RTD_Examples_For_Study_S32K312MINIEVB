...
#define CDD_PLATFORM_IPW_AR_RELEASE_MINOR_VERSION_C          9
#define CDD_PLATFORM_IPW_AR_RELEASE_REVISION_VERSION_C       0
#define CDD_PLATFORM_IPW_SW_MAJOR_VERSION_C                  7
#define CDD_PLATFORM_IPW_SW_MINOR_VERSION_C                  0
#define CDD_PLATFORM_IPW_SW_PATCH_VERSION_C                  1
...
void Platform_Ipw_Init(const Platform_Ipw_ConfigType *pConfig)
{
    ...
    if (pConfig->aSystemConfig != NULL_PTR)
    {
        if ((pConfig->aSystemConfig)->u8SysIrqsCount <= SYSTEM_IP_MAX_CORE_INT)
        {
            ...
            {
                System_Ip_ConfigIrq((pConfig->aSystemConfig)->aSystemIrqs[u8IrqIdx].eIrq, (pConfig->aSystemConfig)->aSystemIrqs[u8IrqIdx].bEnable);
            }
            System_Ip_SetAhbSlavePriority((pConfig->aSystemConfig)->bAhbSlavePriority);
        }
    }
#endif
    ...
    {
        Intm_Ip_SelectMonitoredIrq(pConfig->pIntMonitor->pIntmChannelConfig[u8Idx].eMonitor, pConfig->pIntMonitor->pIntmChannelConfig[u8Idx].eMonitoredIrq);
        Intm_Ip_SetLatency(pConfig->pIntMonitor->pIntmChannelConfig[u8Idx].eMonitor, pConfig->pIntMonitor->pIntmChannelConfig[u8Idx].u32AcceptedLatency);
    }
                                                ...
                                                IRQn_Type eIrqNumber)
{
    ...
    eHwChannel = pConfig->pIntMonitor->pIntmChannelConfig[u8ChannelIdxInPartition].eMonitor;
