...
#define EMIOS_ICU_IP_IRQ_AR_RELEASE_MINOR_VERSION_C        9
#define EMIOS_ICU_IP_IRQ_AR_RELEASE_REVISION_VERSION_C     0
#define EMIOS_ICU_IP_IRQ_SW_MAJOR_VERSION_C                7
...
/**
 @brief      This function is used to read the period values for channels use Bus Diverse
 @details    This function returns the Counter Value
 @param[in]  instance  - eMIOS module index
 @param[in]  hwChannel - hardware channel eMIOS
 @pre        Icu_Init must be called before.
*/
static inline uint32 Emios_Icu_Ip_ReadBusDiverseCounterBus(const uint8 instance,
                                                           const uint8 hwChannel
                                                          );
...
*
*/
static inline void Emios_Icu_Ip_GetDuCyAndPeriodWithSAICMode(const uint8 instance,
                                                             const uint8 hwChannel,
...
/**
* @brief      Emios_Icu function that handles the masterbus interrupt of eMIOS channel.
...
#if ((EMIOS_ICU_IP_TIMESTAMP_API == STD_ON)||(EMIOS_ICU_IP_SIGNAL_MEASUREMENT_API == STD_ON)||(EMIOS_ICU_IP_EDGE_DETECT_API == STD_ON))
/**
* @brief      Emios_Icu function that handles the overflow callback for HLD and IPL of eMIOS channel.
* @details    This function:
*              - Report overflow to notify if necessary
*
* @param[in]  instance  - eMIOS module index
* @param[in]  hwChannel - eMIOS hardware channel
*/
static inline void Emios_Icu_Ip_ProcessReportOverFlow(const uint8 instance,
                                                      const uint8 hwChannel
                                                      );
...
/**
 * @brief      Emios_Icu function that handles the timestamp measurement type interrupt.
                                                          ...
                                                          )
{
    ...
    {
        /* eMIOS Counter Bus_B Channel (EMIOS_ICU_IP_CHANNEL_0) is in Modulous counter buffer or Modulous counter mode */
    ...
    {
        /* eMIOS Counter Bus_C Channel (EMIOS_ICU_IP_CHANNEL_8) is in Modulous counter buffer or Modulous counter mode */
    ...
    {
        /* eMIOS Counter Bus_D Channel (EMIOS_ICU_IP_CHANNEL_16) is in Modulous counter buffer or Modulous counter mode */
                                                ...
                                                )
{
    ...
    switch (u32ChannelEmiosBus)
    {
        ...
        {
            u32Period = Emios_Icu_Ip_ReadBusDiverseCounterBus(instance, hwChannel);
        }
        break;
                                                             ...
                                                             )
{
    ...
    else
    {
        ...
        {
            /* DUTYCYCLE or PERIOD measurement */
            Emios_Icu_Ip_GetDuCyAndPeriodWithSAICMode(instance, hwChannel, Pulse_Width, bOverflow);
                                                      ...
                                                      )
{
    ...
    eMios_Icu_Ip_ModeType nMeasMode = eMios_Icu_Ip_axChState[eMios_Icu_Ip_u8IndexInChState[instance][hwChannel]].channelMode;

    if((EMIOS_ICU_MODE_TIMESTAMP == nMeasMode) || (EMIOS_ICU_MODE_SIGNAL_MEASUREMENT == nMeasMode))
    {
        Emios_Icu_Ip_ReportOverflow(instance, hwChannel, TRUE);
    }
    else if(EMIOS_ICU_MODE_SIGNAL_EDGE_DETECT == nMeasMode)
    {
        /* Calling HLD Report wakeup and overflow for the logical channel */
        if(eMios_Icu_Ip_axChState[eMios_Icu_Ip_u8IndexInChState[instance][hwChannel]].callbackOverflow != NULL_PTR)
        {
            eMios_Icu_Ip_axChState[eMios_Icu_Ip_u8IndexInChState[instance][hwChannel]].callbackOverflow(eMios_Icu_Ip_axChState[eMios_Icu_Ip_u8IndexInChState[instance][hwChannel]].callbackParam, TRUE);
        }
        else
        {
            /* Calling User Overflow Notification for the IPL channel */
            if (NULL_PTR != eMios_Icu_Ip_axChState[eMios_Icu_Ip_u8IndexInChState[instance][hwChannel]].eMiosOverflowNotification)
            {
    ...
    for(nCounter=0U; nCounter < EMIOS_ICU_IP_NUM_OF_CHANNELS; nCounter++)
    {
        if (eMios_Icu_Ip_u8IndexInChState[instance][nCounter] < EMIOS_ICU_IP_NUM_OF_CHANNELS_USED)
        {
            ...
            switch(BusSelected)
            {
                case EMIOS_ICU_BUS_DIVERSE:
                {
                    if((nCounter <= (hwChannel + ((uint8)7U))) && (nCounter >= (hwChannel + ((uint8)1U))))
                    {
            ...
            {
                Emios_Icu_Ip_ProcessReportOverFlow(instance, nCounter);
            }
        }
    }
}
