...
#define EMIOS_ICU_IP_AR_RELEASE_MINOR_VERSION_C        9
#define EMIOS_ICU_IP_AR_RELEASE_REVISION_VERSION_C     0
#define EMIOS_ICU_IP_SW_MAJOR_VERSION_C                7
...
#if ((EMIOS_ICU_IP_TIMESTAMP_API == STD_ON)||(EMIOS_ICU_IP_SIGNAL_MEASUREMENT_API == STD_ON)||(EMIOS_ICU_IP_EDGE_DETECT_API == STD_ON)||(EMIOS_ICU_IP_DEINIT_API == STD_ON))
    #if (EMIOS_ICU_IP_OVERFLOW_NOTIFICATION_API == STD_ON)
/**
...
(
    ...
    #endif /* EMIOS_ICU_IP_OVERFLOW_NOTIFICATION_API == STD_ON */
#endif /* (EMIOS_ICU_IP_TIMESTAMP_API == STD_ON)||(EMIOS_ICU_IP_SIGNAL_MEASUREMENT_API == STD_ON)||(EMIOS_ICU_IP_EDGE_DETECT_API == STD_ON)||(EMIOS_ICU_IP_DEINIT_API == STD_ON) */

#if (EMIOS_ICU_IP_DEINIT_API == STD_ON)
    #if (EMIOS_ICU_IP_OVERFLOW_NOTIFICATION_API == STD_ON)
/**
* @brief      Icu driver function that clear the master buses interrupt flag .
* @details    This function:
*             - Only using for the following functions:
*                   - Emios_Icu_Ip_Deinit: Used for clear master buses interrupt flag that hwChannel uses as an overflow notification.
*
* @param[in]  instance        - eMIOS module index
* @param[in]  hwChannel       - eMIOS encoded hardware channel using master bus
* @api
*/
static inline void Emios_Icu_Ip_ClearMasterBusInterruptFlag
(
    const uint8 instance,
    const uint8 hwChannel
);
    #endif /* EMIOS_ICU_IP_OVERFLOW_NOTIFICATION_API == STD_ON */
#endif /* EMIOS_ICU_IP_DEINIT_API == STD_ON */
    ...
    if (EMIOS_ICU_BUS_INTERNAL_COUNTER != eMios_Icu_Ip_axChState[eMios_Icu_Ip_u8IndexInChState[instance][hwChannel]].BusSelected)
    {
        ...
        if (nMasterBusHwChannel < EMIOS_ICU_IP_NUM_OF_CHANNELS)
        {
            if((uint8)0U == eMios_Icu_Ip_u8NumOvflByCounterBus[instance][nMasterBusHwChannel])
            {
                ...
                s_emiosBase[instance]->CH.UC[nMasterBusHwChannel].S |= (eMIOS_S_OVR_MASK | eMIOS_S_OVFL_MASK | eMIOS_S_FLAG_MASK);
                                                          ...
                                                          const uint8 hwChannel)
{
    ...
    if (EMIOS_ICU_BUS_INTERNAL_COUNTER != eMios_Icu_Ip_axChState[eMios_Icu_Ip_u8IndexInChState[instance][hwChannel]].BusSelected)
    {
        ...
        if (nMasterBusHwChannel < EMIOS_ICU_IP_NUM_OF_CHANNELS)
        {
            if (eMios_Icu_Ip_u8NumOvflByCounterBus[instance][nMasterBusHwChannel] > (uint8)0U)
            {
                eMios_Icu_Ip_u8NumOvflByCounterBus[instance][nMasterBusHwChannel]--;
            }
            if((uint8)0U == eMios_Icu_Ip_u8NumOvflByCounterBus[instance][nMasterBusHwChannel])
...
* @api
*/
static inline void Emios_Icu_Ip_ClearMasterBusInterruptFlag(const uint8 instance,
                                                            const uint8 hwChannel)
{
    uint8 nMasterBusHwChannel = Emios_Icu_Ip_GetMasterBus(hwChannel, eMios_Icu_Ip_axChState[eMios_Icu_Ip_u8IndexInChState[instance][hwChannel]].BusSelected);
    /* Clear pending interrupts */
    s_emiosBase[instance]->CH.UC[nMasterBusHwChannel].S |= (eMIOS_S_OVR_MASK | eMIOS_S_OVFL_MASK | eMIOS_S_FLAG_MASK);
    ...
    if (FALSE == eMios_Icu_Ip_bInstanceState[instance])
    {
        ...
        if (userConfig->nNumChannels > 0U)
        {
            ...
            for (index=0U; index < userConfig->nNumChannels; index++)
            {
                ...
                eMios_Icu_Ip_axChState[eMios_Icu_Ip_u8IndexInChState[instance][hwChannel]].callbackOverflow = (*userConfig->pChannelsConfig)[index].callbackOverflow;
...
#endif /* (STD_ON == EMIOS_ICU_USES_MCL_DRIVER) */

#if (EMIOS_ICU_IP_OVERFLOW_NOTIFICATION_API == STD_ON)
                        /* Clear pending interrupts */
                        Emios_Icu_Ip_ClearMasterBusInterruptFlag(instance, hwChannel);
#endif /* (EMIOS_ICU_IP_OVERFLOW_NOTIFICATION_API == STD_ON) */
                    }
