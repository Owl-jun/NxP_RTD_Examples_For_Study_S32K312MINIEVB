...
#define RTC_IP_AR_RELEASE_MINOR_VERSION_C     9
#define RTC_IP_AR_RELEASE_REVISION_VERSION_C  0
#define RTC_IP_SW_MAJOR_VERSION_C             7
#define RTC_IP_SW_MINOR_VERSION_C             0
#define RTC_IP_SW_PATCH_VERSION_C             1
...
static inline boolean Rtc_Ip_CheckBitIsZero(uint32 value, uint32 mark);
#endif
...
static inline uint32 Rtc_Ip_GetStatusRegister(uint8 instance);
static inline uint32 Rtc_Ip_GetControlRegister(uint8 instance);
...
/**
 * @brief
 * Function Name : Rtc_Ip_CheckBitIsZero
 * Description   : Check bit is zero?
 *
 * @param[in]   value        value to check
 * @param[in]   mark
 * @return  boolean value is true or false
 * @pre     The driver needs to be initialized.
 */
static inline boolean Rtc_Ip_CheckBitIsZero(uint32 value, uint32 mark)
{
    boolean returnValue = (0U != (value & mark)) ? TRUE : FALSE;
...
/**
 * @brief
 * Function Name : Rtc_Ip_GetStatusRegister
 * Description   : Get the value of RTC Status register (RTCS)
 *
 * @param[in]   instance        RTC hw instance number
 *
 * @return  uint32 value of RTC Status register (RTCS)
 * @pre     The driver needs to be initialized.
 */
static inline uint32 Rtc_Ip_GetStatusRegister(uint8 instance)
{
    uint32 returnValue = rtcBase[instance]->RTCS;
...
/**
 * @brief
 * Function Name : Rtc_Ip_GetControlRegister
 * Description   : Get the value of RTC Control register (RTCC)
 *
 * @param[in]   instance        RTC hw instance number
 *
 * @return  uint32 value of RTC Control register (RTCC)
 * @pre     The driver needs to be initialized.
 */
static inline uint32 Rtc_Ip_GetControlRegister(uint8 instance)
{
    uint32 returnValue = rtcBase[instance]->RTCC;
...
static void Rtc_Ip_ProcessInterrupt(uint8 instance)
{
    ...
    if (instance < RTC_INSTANCE_COUNT)
    {
        ...
        {
            uint32 RTCS_Value = Rtc_Ip_GetStatusRegister(instance);
			uint32 RTCC_Value = Rtc_Ip_GetControlRegister(instance);

            apiInterruptStatusFlag = Rtc_Ip_CheckBitIsZero(RTCS_Value, RTC_RTCS_APIF_MASK);
            apiInterruptEnableFlag = Rtc_Ip_CheckBitIsZero(RTCC_Value, RTC_RTCC_APIIE_MASK);
            rtcInterruptStatusFlag = Rtc_Ip_CheckBitIsZero(RTCS_Value, RTC_RTCS_RTCF_MASK);
            rtcInterruptEnableFlag = Rtc_Ip_CheckBitIsZero(RTCC_Value, RTC_RTCC_RTCIE_MASK);
            rollOverInterruptStatusFlag = Rtc_Ip_CheckBitIsZero(RTCS_Value, RTC_RTCS_ROVRF_MASK);
            rollOverInterruptEnableFlag = Rtc_Ip_CheckBitIsZero(RTCC_Value, RTC_RTCC_ROVREN_MASK);
...
* @details       This function:
*                        - get APIVAL register value
*
* @param[in]     instance        Rtc hw instance
* @return        uint32
* @pre           The driver needs to be initialized.
* @implements    Rtc_Ip_GetAPICompareRegister_Activity
*
*/
...
* @brief         Rtc Driver function
* @details       This function:
*                        - get RTCCNT register value
*
* @param[in]     instance        Rtc hw instance
* @return        uint32
* @pre           The driver needs to be initialized.
* @implements    Rtc_Ip_GetCounterRegister_Activity
*
*/
...
* @return        void
* @pre           The driver needs to be initialized.
* @implements    Rtc_Ip_TriggerEnable_Activity
*
*/
