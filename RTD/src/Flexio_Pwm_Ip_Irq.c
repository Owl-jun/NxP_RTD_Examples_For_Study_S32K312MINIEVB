...
#define FLEXIO_PWM_IP_IRQ_AR_RELEASE_MINOR_VERSION_C       9
#define FLEXIO_PWM_IP_IRQ_AR_RELEASE_REVISION_VERSION_C    0
#define FLEXIO_PWM_IP_IRQ_SW_MAJOR_VERSION_C               7
#define FLEXIO_PWM_IP_IRQ_SW_MINOR_VERSION_C               0
#define FLEXIO_PWM_IP_IRQ_SW_PATCH_VERSION_C               1
                              ...
                              )
{
    ...
    boolean PinState = Flexio_Pwm_Ip_GetPinState(
                                            ...
                                            Flexio_Pwm_Ip_aState[0U][ChannelId]->PinId);

    /* Check if it is a pin interrupt */
    if ((PinFlags & Flexio_Pwm_Ip_u32PinIrqMask & (1U << Flexio_Pwm_Ip_aState[0U][ChannelId]->PinId)) != 0U)
    {
        if(NULL_PTR != Flexio_Pwm_Ip_aState[0U][ChannelId]->IplCallback.CbFunction)
        {
            if (TRUE ==  PinState)
            {
            ...
            Flexio_Pwm_Ip_aState[0U][ChannelId]->IplCallback.CbFunction(
                                        IrqType,
                                        Flexio_Pwm_Ip_aState[0U][ChannelId]->IplCallback.CbParameter);
        }
        else
        {
            if(NULL_PTR != Flexio_Pwm_Ip_aState[0U][ChannelId]->HldCallback.CbFunction)
            {
                Flexio_Pwm_Ip_aState[0U][ChannelId]->HldCallback.CbFunction(
                            Flexio_Pwm_Ip_aState[0U][ChannelId]->HldCallback.CbParameter);
            }
        }
    }
#endif
    /* Find what type of interrupt occurred */
    if ((TimerFlags & Flexio_Pwm_Ip_u8TimerIrqMask & (1U << Flexio_Pwm_Ip_aState[0U][ChannelId]->TimerId)) != 0U)
    {
        if(NULL_PTR != Flexio_Pwm_Ip_aState[0U][ChannelId]->IplCallback.CbFunction)
        {
